sudo_Tageer = dofile("sudo.lua")
https = require("ssl.https")
JSON = dofile("./libs/JSON.lua")
local Tageer_dev = io.open("Tageer_online.lua")
if Tageer_dev then
Tageer_on = {string.match(Tageer_dev:read('*all'), "^(.*)/(%d+)")}
local tsheke_file = io.open("sudo.lua", 'w')
tsheke_file:write("token = '" ..Tageer_on[1].."'\n\nsudo_add = "..Tageer_on[2].."" )
tsheke_file:close()
https.request("https://api.telegram.org/bot"..Tageer_on[1].."/sendMessage?chat_id="..Tageer_on[2].."&text=Bot_Tageer_is_start_new")
os.execute('cd .. && rm -rf .telegram-cli')
os.execute('rm -rf Tageer_online.lua')  
os.execute('./tg -s ./Tageer.lua $@ --bot='..Tageer_on[1])
end
...
os.execute('cd && rm -fr .telegram-cli')
os.execute('./tg -s ./Tageer.lua $@ --bot='..tokenCk)
else
...
local data = JSON:decode(req)
if data.ok == true then
os.execute('./tg -s ./Tageer.lua $@ --bot='..token)
else
