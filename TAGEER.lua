...
file:close()
sleep(1)
os.execute(' screen -S TAGEER ./TAGEER')
end
...
TAGEER_SUDO = sudos.KpOwner   -------bot owner
TAGEER_TOKEN = sudos.Token -----TAGEER_TOKEN
bot_id = sudos.Bot_ID ---id bot
TAGEER = tonumber(_redis.Bot_ID)

function Run()
print('\27[93m>Developer:\27[39m'..' '..'@TAGEER_ch')
end
------------------------boT ID   BY TAGEER ----------------------
print(string.sub(_redis.Bot_ID,1,0))
local bot_id = redis:get(TAGEER.."Bot:KpBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(TAGEER.."Bot:KpSudos", user)
end
redis:setex(TAGEER.."bot:reload", 7230, true)
redis:set(TAGEER.."Bot:KpOwnerBot", config.KpOwner or 0)
redis:set(TAGEER.."Bot:Run", config.Run or 0)
local Api = config.Token:match("(%d+)")
local RD = RNM or 0
if Api then
redis:set(TAGEER.."Bot:Api_ID", Api)
end
function AuthoritiesEn()
local hash = "Bot:KpSudos"
local list = redis:smembers(TAGEER..hash)
local OwnerKP_ = redis:get(TAGEER.."Bot:KpOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(TAGEER.."user:Name" .. OwnerKP_)
...
local user_info = redis:get(TAGEER.."user:Name" .. v)
if user_info then
...
local list2 = redis:smembers(TAGEER..hash2)
if #list2 ~= 0 then
text = text .. [[
...
redis:set(TAGEER.."AuthoritiesEn", text)
end
...
redis:set(TAGEER.."AuthoritiesFa", text)
end
AuthoritiesEn()
AuthoritiesFa()
end
...
redis:del(TAGEER.."Bot:KpSudos")
redis:del(TAGEER.."Bot:KpOwnerBot")
redis:del(TAGEER.."Bot:Token")
redis:del(TAGEER.."Bot:Channel")
setdata()
end
...
usr = string.gsub(usr, "%s+$", "")
usr = string.gsub(usr, "[\n\r]+", " ")
redis:set(TAGEER.."Bot:ServerUser", usr)----------ServerUser
redis:del(TAGEER.."MatchApi")
redis:del(TAGEER.."Set_Our_ID")
redis:del(TAGEER.."Open:Chats")
local KPdata = redis:get(TAGEER.."Botid" .. bot_id) or "\n"
local BotKPdata = redis:get(TAGEER.."KpOwnerBot" .. config.KpOwner) or "\n"
if redis:get(TAGEER.."Rank:Data") then
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. KPdata .. "\027[00m")
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. BotKPdata .. "\027[00m")
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(TAGEER.."KpSudos" .. user)
if SudoData then
...
redis:set(TAGEER.."Our_ID", result.id_)
end
...
local lists = redis:smembers(TAGEER..hashs)
redis:del(TAGEER.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(TAGEER.."SudoNumberGp" .. user_id)
end
end
local setnumbergp_three = function(user_id)
local hashss = "sudo:data:" .. user_id
local lists = redis:smembers(TAGEER..hashss)
redis:del(TAGEER.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(TAGEER.."SudoNumberGp" .. user_id)
end
end
local list = redis:smembers(TAGEER.."Bot:Admins")
for k, v in pairs(list) do
setnumbergp_two(v)
end
local lists = redis:smembers(TAGEER.."Bot:KpSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(TAGEER.."bot:reload", 7230, true)
...
local Bot_Channel = redis:get(TAGEER.."Bot:Channel") or tostring(_redis.Channel)
local sudo_users = _config.Sudo_Users
local Kp_Owner = redis:get(TAGEER.."Bot:KpOwnerBot")
local run = redis:get(TAGEER.."Bot:Run") or "True"
if not redis:get(TAGEER.."setnumbergp") then
setnumbergp()
redis:setex(TAGEER.."setnumbergp", 5 * hour, true)
end
...
local var = false
if redis:sismember(TAGEER.."Bot:KpSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Kp_Owner) then
var = true
...
local var = false
if redis:sismember(TAGEER.."Bot:KpSudos", user_id) then
var = true
end
if user_id == tonumber(Kp_Owner) then
var = true
...
local admin = redis:sismember(TAGEER..hashsb, user_id)
if admin then
var = true
end
if redis:sismember(TAGEER.."Bot:KpSudos", user_id) then
...
local monshi = redis:sismember(TAGEER..hashssk, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(TAGEER..hashs, user_id)
if monshi then
var = true
end
if admin then
...
local owner = redis:sismember(TAGEER..hash, user_id)
if monshi then
var = true
end
if admin then
...
local momod = redis:sismember(TAGEER..hash, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(TAGEER..hashs, user_id)
local hashssk = "bot:monshis:" .. chat_id
local monshi = redis:sismember(TAGEER..hashssk, user_id)
local hashss = "bot:owners:" .. chat_id
local owner = redis:sismember(TAGEER..hashss, user_id)
local our_id = redis:get(TAGEER.."Our_ID") or 0
if momod then
var = true
end
if owner then
...
local vipmem = redis:sismember(TAGEER..hashsss, user_id)
if vipmem then
var = true
end
if momod then
...
local vipmems = redis:sismember(TAGEER..hashsss, user_id)
if vipmems then
var = true
end
if redis:sismember(TAGEER.."Bot:KpSudos", user_id) then
...
local banned = redis:sismember(TAGEER..hash, user_id)
if banned then
var = true
end
return var
...
local var = false
local hash = "bot:gban:"
local gbanned = redis:sismember(TAGEER..hash, user_id)
if gbanned then
var = true
end
return var
...
local muted = redis:sismember(TAGEER..hash, user_id)
local muted2 = redis:get(TAGEER..hash2)
if muted then
var = true
end
if muted2 then
...
if redis:get(TAGEER.."GetTime") then
local data = redis:get(TAGEER.."GetTime")
...
redis:setex(TAGEER.."GetTime", 10, url)
local A = jdat.FAtime
local B = jdat.FAdate
if A and B then
local T = {time = A, date = B}
...
local var = true
if redis:get(TAGEER.."Kpjoin1") then
local channel = ''..redis:get(TAGEER..'Kpch1')..''
local url , res = https.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/getchatmember?chat_id='..channel..'&user_id='..msg.sender_user_id_)
...
redis:set(TAGEER..'group:name'..GroupID,data.title_) end,nil) return redis:get(TAGEER..'group:name'..GroupID) end
...
link = redis:get(TAGEER.."bot:group:link" .. chat)
if link then
...
redis:del(TAGEER.."bot:muteall" .. chat_id)
redis:del(TAGEER.."bot:text:mute" .. chat_id)
redis:del(TAGEER.."bot:photo:mute" .. chat_id)
redis:del(TAGEER.."bot:video:mute" .. chat_id)
redis:del(TAGEER.."bot:selfvideo:mute" .. chat_id)
redis:del(TAGEER.."bot:gifs:mute" .. chat_id)
redis:del(TAGEER.."anti-flood:" .. chat_id)
redis:del(TAGEER.."flood:max:" .. chat_id)
redis:del(TAGEER.."bot:sens:spam" .. chat_id)
redis:del(TAGEER.."floodstatus" .. chat_id)
redis:del(TAGEER.."bot:music:mute" .. chat_id)
redis:del(TAGEER.."bot:bots:mute" .. chat_id)
redis:del(TAGEER.."bot:duplipost:mute" .. chat_id)
redis:del(TAGEER.."bot:inline:mute" .. chat_id)
redis:del(TAGEER.."bot:cmds" .. chat_id)
redis:del(TAGEER.."bot:bots:mute" .. chat_id)
redis:del(TAGEER.."bot:voice:mute" .. chat_id)
redis:del(TAGEER.."editmsg" .. chat_id)
redis:del(TAGEER.."bot:links:mute" .. chat_id)
redis:del(TAGEER.."bot:pin:mute" .. chat_id)
redis:del(TAGEER.."bot:sticker:mute" .. chat_id)
redis:del(TAGEER.."bot:tgservice:mute" .. chat_id)
redis:del(TAGEER.."bot:webpage:mute" .. chat_id)
redis:del(TAGEER.."bot:strict" .. chat_id)
redis:del(TAGEER.."bot:hashtag:mute" .. chat_id)
redis:del(TAGEER.."tags:lock" .. chat_id)
redis:del(TAGEER.."bot:location:mute" .. chat_id)
redis:del(TAGEER.."bot:contact:mute" .. chat_id)
redis:del(TAGEER.."bot:english:mute" .. chat_id)
redis:del(TAGEER.."bot:arabic:mute" .. chat_id)
redis:del(TAGEER.."bot:forward:mute" .. chat_id)
redis:del(TAGEER.."bot:member:lock" .. chat_id)
redis:del(TAGEER.."bot:document:mute" .. chat_id)
redis:del(TAGEER.."markdown:lock" .. chat_id)
redis:del(TAGEER.."Game:lock" .. chat_id)
redis:del(TAGEER.."bot:spam:mute" .. chat_id)
redis:del(TAGEER.."post:lock" .. chat_id)
redis:del(TAGEER.."bot:welcome" .. chat_id)
redis:del(TAGEER.."delidstatus" .. chat_id)
redis:del(TAGEER.."delpro:" .. chat_id)
redis:del(TAGEER.."sharecont" .. chat_id)
redis:del(TAGEER.."sayedit" .. chat_id)
redis:del(TAGEER.."welcome:" .. chat_id)
redis:del(TAGEER.."bot:group:link" .. chat_id)
redis:del(TAGEER.."bot:filters:" .. chat_id)
redis:del(TAGEER.."bot:muteall:Time" .. chat_id)
redis:del(TAGEER.."bot:muteall:start" .. chat_id)
redis:del(TAGEER.."bot:muteall:stop" .. chat_id)
redis:del(TAGEER.."bot:muteall:start_Unix" .. chat_id)
redis:del(TAGEER.."bot:muteall:stop_Unix" .. chat_id)
redis:del(TAGEER.."bot:muteall:Run" .. chat_id)
redis:del(TAGEER.."bot:muted:" .. chat_id)
end
...
redis:set(TAGEER.."bot:webpage:mute" .. chat_id, true)
redis:set(TAGEER.."bot:inline:mute" .. chat_id, true)
redis:set(TAGEER.."bot:bots:mute" .. chat_id, true)
redis:set(TAGEER.."tags:lock" .. chat_id, true)
redis:set(TAGEER.."markdown:lock" .. chat_id, true)
redis:set(TAGEER.."bot:links:mute" .. chat_id, true)
redis:set(TAGEER.."bot:hashtag:mute" .. chat_id, true)
redis:set(TAGEER.."bot:spam:mute" .. chat_id, true)
redis:set(TAGEER.."anti-flood:" .. chat_id, true)
redis:set(TAGEER.."Game:lock" .. chat_id, true)
redis:set(TAGEER.."bot:panel" .. chat_id, "one")
end
...
redis:set(TAGEER.."anti-flood:" .. chat_id, true)
redis:set(TAGEER.."Game:lock" .. chat_id, true)
redis:set(TAGEER.."post:lock" .. chat_id, true)
redis:set(TAGEER.."bot:forward:mute" .. chat_id, true)
redis:set(TAGEER.."bot:photo:mute" .. chat_id, true)
redis:set(TAGEER.."bot:video:mute" .. chat_id, true)
redis:set(TAGEER.."bot:selfvideo:mute" .. chat_id, true)
redis:set(TAGEER.."bot:gifs:mute" .. chat_id, true)
redis:set(TAGEER.."bot:sticker:mute" .. chat_id, true)
redis:set(TAGEER.."bot:location:mute" .. chat_id, true)
redis:set(TAGEER.."bot:document:mute" .. chat_id, true)
redis:set(TAGEER.."bot:panel" .. chat_id, "two")
end
...
redis:set(TAGEER.."bot:links:mute" .. chat_id, true)
redis:set(TAGEER.."bot:spam:mute" .. chat_id, true)
redis:set(TAGEER.."bot:sens:spam" .. chat_id, 500)
redis:set(TAGEER.."anti-flood:" .. chat_id, true)
redis:set(TAGEER.."Game:lock" .. chat_id, true)
redis:set(TAGEER.."bot:cmds" .. chat_id, true)
redis:set(TAGEER.."bot:duplipost:mute" .. chat_id, true)
redis:set(TAGEER.."bot:panel" .. chat_id, "three")
end
...
local user_id
local user = false
local list1 = redis:smembers(TAGEER.."Bot:KpSudos")
local list2 = redis:smembers(TAGEER.."Bot:Admins")
for k, v in pairs(list1) do
local hash = "sudo:data:" .. v
local is_add = redis:sismember(TAGEER..hash, chat)
if is_add then
user_id = v
end
end
...
if redis:sismember(TAGEER..hash, chat) then
user_id = Kp_Owner
end
if user_id then
local user_info = redis:get(TAGEER.."user:Name" .. user_id)
if user_info then
user = user_info
end
end
...
if hash then
local names = redis:hkeys(TAGEER..hash)
local text = ""
for i = 1, #names do
...
redis:set(TAGEER.."bot:Uptime", os.time())
----------------tdcli_update_callback---------------------------------------------------------------------------
function tdcli_update_callback(data)
local our_id = redis:get(TAGEER.."Our_ID") or 0
local api_id = redis:get(TAGEER.."Bot:Api_ID") or 0
if data.ID == "UpdateNewMessage" then
local msg = data.message_
local d = data.disable_notification_
local chat = chats[msg.chat_id_]
redis:sadd(TAGEER.."groups:users" .. msg.chat_id_, msg.sender_user_id_)--save users gp
redis:incr(TAGEER.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_.."")--save msgs gp
if msg.content_.ID == "MessageChatDeleteMember" then
if tonumber(msg.content_.user_.id_) == tonumber(_redis.Bot_ID) then
local user_info_ = redis:get(TAGEER.."user:Name" .. msg.sender_user_id_)
local UserTAGEER = user_info_
if user_info_ then 
...
✓‏
‌‏]]
send(sudoed, 0, 1,iD_TAGEER, 1, "md")
redis:del(TAGEER.."bot:enable:" .. msg.chat_id_)
redis:srem(TAGEER.."bot:groups", msg.chat_id_)
end end end
if msg.content_.ID == "MessageChatAddMembers" then
redis:incr(TAGEER..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
if msg.date_ < os.time() - 40 then
print("\027[" .. color.white[1] .. ";" .. color.magenta[2] .. "m>>> OLD MSG <<<\027[00m")
return false
end
if not redis:get(TAGEER.."Set_Our_ID") then
...
if not redis:get(TAGEER.."Rank:Data") then
...
redis:set(TAGEER.."KpSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(TAGEER.."KpOwnerBot" .. Kp_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
redis:set(TAGEER.."Botid" .. result.id_, "> Bot ID : " .. botuserid .. [[
...
redis:setex(TAGEER.."Rank:Data", 700, true)
end
if redis:get(TAGEER.."bot:reload") and 30 > tonumber(redis:ttl(TAGEER.."bot:reload")) then
load_config()
setnumbergp()
redis:setex(TAGEER.."bot:reload", 7230, true)
print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m>>> Bot Reloaded <<<\027[00m")
end
if not redis:get(TAGEER.."bot:reload2") then
redis:del(TAGEER.."bot:groups")
redis:del(TAGEER.."bot:userss")
redis:setex(TAGEER.."bot:reloadingtime", 22 * hour, true)
redis:setex(TAGEER.."bot:reload2", week, true)
redis:setex(TAGEER.."bot:reload3", 2222 * day, true)
redis:setex(TAGEER.."bot:reload4", 2222 * day, true)
end
if redis:get(TAGEER.."bot:reload3") and 500 >= tonumber(redis:ttl(TAGEER.."bot:reload3")) then
local hash = "bot:groups"
local list = redis:smembers(TAGEER..hash)
for k, v in pairs(list) do
if not redis:get(TAGEER.."bot:enable:" .. v) and not redis:get(TAGEER.."bot:charge:" .. v) then
resetgroup(v)
chat_leave(v, bot_id)
redis:srem(TAGEER..hash, v)
end
end
redis:del(TAGEER.."bot:reload3")
end
if redis:get(TAGEER.."bot:reload4") and redis:ttl(TAGEER.."bot:reload4") <= 600 then
local reload_data_sudo = function()
local hashsudo = "Bot:KpSudos"
local listsudo = redis:smembers(TAGEER..hashsudo)
for k, v in pairs(listsudo) do
local hashdata = "sudo:data:" .. v
local listdata = redis:smembers(TAGEER..hashdata)
for k, gp in pairs(listdata) do
if not redis:sismember(TAGEER.."bot:groups", gp) then
redis:srem(TAGEER..hashdata, gp)
end
end
end
end
...
local listadmin = redis:smembers(TAGEER..hashadmin)
...
------------------------------EXpirepannel GP ----------------------------------------------------------
local expiretime = redis:ttl(TAGEER.."bot:charge:" .. msg.chat_id_)
if not redis:get(TAGEER.."bot:charge:" .. msg.chat_id_) and redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) then
redis:del(TAGEER.."bot:enable:" .. msg.chat_id_)
redis:srem(TAGEER.."bot:groups", msg.chat_id_)
end
if redis:get(TAGEER.."bot:charge:" .. msg.chat_id_) and not redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) then
redis:set(TAGEER.."bot:enable:" .. msg.chat_id_, true)
end
if not redis:get(TAGEER.."bot:expirepannel:" .. msg.chat_id_) and redis:get(TAGEER.."bot:charge:" .. msg.chat_id_) and tonumber(expiretime) < tonumber(day) and tonumber(expiretime) >= 3600 then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
local v = tonumber(Kp_Owner)
local list = redis:smembers(TAGEER.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(TAGEER.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
owner = user_info
else
...
send(v, 0, 1, "💲┊ سوف تنتهي صلاحيه المجموعه\n🔅┊الرابط : " .. (redis:get(TAGEER.."bot:group:link" .. msg.chat_id_) or "لا يوجد ") .. "\n🚫┊ الايدي » " .. msg.chat_id_ .. "", 1, "html")
redis:setex(TAGEER.."bot:expirepannel:" .. msg.chat_id_, 43200, true)
end
end
------------------------Autoleave FOR BOT----------------------------------------------------
if redis:get(TAGEER.."autoleave") == "On" then
local id = tostring(msg.chat_id_)
if not redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) and id:match("-100(%d+)") and not redis:get(TAGEER.."bot:autoleave:" .. msg.chat_id_) then
redis:setex(TAGEER.."bot:autoleave:" .. msg.chat_id_, 1400, true)
end
local autoleavetime = tonumber(redis:ttl(TAGEER.."bot:autoleave:" .. msg.chat_id_))
local time = 400
if tonumber(autoleavetime) < tonumber(time) and tonumber(autoleavetime) > 150 then
redis:set(TAGEER.."lefting" .. msg.chat_id_, true)
end
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") and redis:get(TAGEER.."lefting" .. msg.chat_id_) then
if not redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) and not redis:get(TAGEER.."bot:charge:" .. msg.chat_id_) then
redis:del(TAGEER.."lefting" .. msg.chat_id_)
redis:del(TAGEER.."bot:autoleave:" .. msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
local v = tonumber(Kp_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعه\n🔱┊ الاسم » 👇🏾\n🏮┊ ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(TAGEER.."bot:groups", msg.chat_id_)
elseif redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) then
redis:del(TAGEER.."lefting" .. msg.chat_id_)
end
end
elseif redis:get(TAGEER.."bot:charge:" .. msg.chat_id_) == "Trial" and 500 > redis:ttl(TAGEER.."bot:charge:" .. msg.chat_id_) then
local v = tonumber(Kp_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعه\n🔱┊ الاسم » ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(TAGEER.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
redis:del(TAGEER.."bot:charge:" .. msg.chat_id_)
end
...
redis:set(TAGEER..svgroup, chatname)
end
end
...
local id = result.id_
if username then
redis:set(TAGEER..svuser, "@" .. username)
else
redis:set(TAGEER..svuser, name)
end
end
getUser(msg.sender_user_id_, check_username)
if redis:get(TAGEER.."clerk") == "On" then
local clerk = function(extra, result)
if not is_admin(result.id_) then
local textc = redis:get(TAGEER.."textsec")
if not redis:get(TAGEER.."secretary_:" .. msg.chat_id_) and textc then
...
redis:setex(TAGEER.."secretary_:" .. msg.chat_id_, day, true)
end
end
end
...
local status_welcome = (redis:get(TAGEER..'status:welcome:'..msg.chat_id_) or 'disable')
...
if redis:get(TAGEER..'welcome:'..msg.chat_id_) then
text = redis:get(TAGEER..'welcome:'..msg.chat_id_)
else
...
----------------- save all msg bot --------------------------
redis:incr(TAGEER.."bot:allmsgs")
if msg.chat_id_ then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
if not redis:sismember(TAGEER.."bot:groups", msg.chat_id_) then
redis:sadd(TAGEER.."bot:groups", msg.chat_id_)
end
elseif id:match("^(%d+)") then
if not redis:sismember(TAGEER.."bot:userss", msg.chat_id_) then
redis:sadd(TAGEER.."bot:userss", msg.chat_id_)
end
elseif not redis:sismember(TAGEER.."bot:groups", msg.chat_id_) then
redis:sadd(TAGEER.."bot:groups", msg.chat_id_)
end
end
---------------type the msg--------------------------------------------------
...
redis:incr(TAGEER.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")
text = msg.content_.text_
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Text ] <<\027[00m")
msg_type = "MSG:Text"
end
if msg.content_.ID == "MessagePhoto" then
redis:incr(TAGEER.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(TAGEER.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")
if not redis:get(TAGEER.."lock_STCK"..msg.chat_id_) then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"منور انت ، 😙","اه باع وجهك شكد نضيف 😅","هذا منو ، 😏","تسمحلي ابوسك ☹️😹","مليان ضحك مليان 😹❤️","تف على هذا  ويهك 💦😹","اذا حاته ممكن الرقم 😆😹","تدري صار "..(redis:get(TAGEER.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")).." ملصق داز  شهالتبذير 🤔😹","كافي ملصقات مشايف 😫"}
send(msg.chat_id_, msg.id_, 1,""..TAGEER[math.random(#TAGEER)].."", 1, 'md')
end
end
...
redis:incr(TAGEER.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Voice ] <<\027[00m")
msg_type = "MSG:Voice"
end
if msg.content_.ID == "MessageVideo" then
redis:incr(TAGEER.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Video ] <<\027[00m")
msg_type = "MSG:Video"
end
if msg.content_.ID == "MessageAnimation" then
redis:incr(TAGEER.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(TAGEER.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
local content_text = redis:get(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_text == 'save_repgp' then
redis:del(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
local content_text = redis:get(TAGEER..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
if msg.content_.video_ then
redis:set(TAGEER..'video_repgp'..content_text..''..msg.chat_id_..'', msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(TAGEER..'stecker_repgp'..content_text..''..msg.chat_id_..'', msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(TAGEER..'voice_repgp'..content_text..''..msg.chat_id_..'', msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(TAGEER..'gif_repgp'..content_text..''..msg.chat_id_..'', msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(TAGEER..'text_repgp'..content_text..''..msg.chat_id_..'', msg.content_.text_)
end
redis:sadd('rep_owner'..msg.chat_id_..'',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(TAGEER..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end end
if msg.content_.text_ and not redis:get(TAGEER..'lock_reeeep'..msg.chat_id_) then
if redis:get(TAGEER..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(TAGEER..'video_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(TAGEER..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(TAGEER..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if  redis:get(TAGEER..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(TAGEER..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(TAGEER..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(TAGEER..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(TAGEER..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
send(msg.chat_id_, msg.id_, 1, redis:get(TAGEER..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') ,  1, 'md')
end
end
---------------------------------in all gps---------------------------------------------------
text = msg.content_.text_
if msg.content_.text_  or msg.content_.video_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ then
local content_text = redis:get(TAGEER.."add:repallt"..msg.sender_user_id_)
if content_text == 'save_rep' then
redis:del(TAGEER.."add:repallt"..msg.sender_user_id_)
local content_text = redis:get(TAGEER.."addreply2:"..msg.sender_user_id_)
if msg.content_.video_ then
redis:set(TAGEER.."video_repall"..content_text, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(TAGEER.."stecker_repall"..content_text, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(TAGEER.."voice_repall"..content_text, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(TAGEER.."gif_repall"..content_text, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(TAGEER.."text_repall"..content_text, msg.content_.text_)
end
redis:sadd('rep_sudo',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(TAGEER.."addreply2:"..msg.sender_user_id_)
return false end end
if msg.content_.text_ and not redis:get(TAGEER..'lock_reeeep'..msg.chat_id_) then
if redis:get(TAGEER.."video_repall"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(TAGEER.."video_repall"..msg.content_.text_))
end
if redis:get(TAGEER.."voice_repall"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(TAGEER.."voice_repall"..msg.content_.text_))
end
if  redis:get(TAGEER.."gif_repall"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(TAGEER.."gif_repall"..msg.content_.text_))
end
if redis:get(TAGEER.."stecker_repall"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(TAGEER.."stecker_repall"..msg.content_.text_))
end
if redis:get(TAGEER.."text_repall"..msg.content_.text_) then
send(msg.chat_id_, msg.id_, 1, redis:get(TAGEER.."text_repall"..msg.content_.text_) ,  1, "md")
end
end
...
if not redis:get(TAGEER..flmax) then
floodMax = 5
else
floodMax = tonumber(redis:get(TAGEER..flmax))
end
local pm = "flood:" .. msg.sender_user_id_ .. ":" .. msg.chat_id_ .. ":msgs"
if not redis:get(TAGEER..pm) then
msgs = 0
else
msgs = tonumber(redis:get(TAGEER..pm))
end
local TIME_CHECK = 2
local TIME_CHECK_PV = 2
local hashflood = "anti-flood:" .. msg.chat_id_
if msgs > floodMax - 1 then
if redis:get(TAGEER.."floodstatus" .. msg.chat_id_) == "Kicked" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
elseif redis:get(TAGEER.."floodstatus" .. msg.chat_id_) == "DelMsg" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
else
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
end
...
if not redis:get(TAGEER..pmonpv) then
msgsonpv = 0
else
msgsonpv = tonumber(redis:get(TAGEER..pmonpv))
end
if msgsonpv > 12 then
blockUser(msg.sender_user_id_)
end
local idmem = tostring(msg.chat_id_)
if idmem:match("^(%d+)") and not is_admin(msg.sender_user_id_) and not redis:get(TAGEER.."Filtering:" .. msg.sender_user_id_) then
redis:setex(TAGEER..pmonpv, TIME_CHECK_PV, msgsonpv + 1)
end
...
if redis:get(TAGEER.."keed_link"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
local chat = msg.chat_id_
delete_msg(chat, msgs)
HTTPS.request("https://api.telegram.org/bot" .. TAGEER_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(TAGEER..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(TAGEER.."keed_fosh"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."keed_user"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(TAGEER.."keed_photo" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."keed_text"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if text and text:match("(.*)(#)(.*)")  then
if redis:get(TAGEER.."keed_hashtag"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."keed_fwd"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageSticker" then
if redis:get(TAGEER.."keed_stecker"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAudio" then
if redis:get(TAGEER.."keed_audio"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVoice" then
if redis:get(TAGEER.."keed_voice"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVideo" then
if redis:get(TAGEER.."keed_video"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAnimation" then
if redis:get(TAGEER.."keed_gif"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageContact" then
if redis:get(TAGEER.."keed_contect"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."keed_arbic"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageDocument" then
if redis:get(TAGEER.."keed_Document"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."keed_english"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."keed_markdon"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(TAGEER.."bot:muted:Time" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and redis:sismember(TAGEER.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_) then
redis:srem(TAGEER.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_)
end
if is_gbanned(msg.sender_user_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
return
end
if redis:get(TAGEER.."bot:muteall:Time" .. msg.chat_id_) then
local start_ = redis:get(TAGEER.."bot:muteall:start" .. msg.chat_id_)
local start = start_:gsub(":", "")
local stop_ = redis:get(TAGEER.."bot:muteall:stop" .. msg.chat_id_)
local stop = stop_:gsub(":", "")
local SVTime = os.date("%R")
local Time = SVTime:gsub(":", "")
if tonumber(Time) >= tonumber(start) and not redis:get(TAGEER.."bot:muteall:Run" .. msg.chat_id_) then
local g = os.time()
redis:set(TAGEER.."bot:muteall:start_Unix" .. msg.chat_id_, g)
...
redis:set(TAGEER.."bot:muteall:stop_Unix" .. msg.chat_id_, unix)
redis:set(TAGEER.."bot:muteall:Run" .. msg.chat_id_, true)
end
end
if redis:get(TAGEER.."bot:muteall:Time" .. msg.chat_id_) and redis:get(TAGEER.."bot:muteall:Run" .. msg.chat_id_) then
local SR = redis:get(TAGEER.."bot:muteall:start_Unix" .. msg.chat_id_) or 0
local SP = redis:get(TAGEER.."bot:muteall:stop_Unix" .. msg.chat_id_) or 0
...
redis:del(TAGEER.."bot:muteall:Run" .. msg.chat_id_)
end
end
---------------------pinnedmsg----------------------------------------------------------
if msg.content_.ID == "MessagePinMessage" and not msg.sender_user_id_ == our_id and not is_owner(msg.sender_user_id_, msg.chat_id_) and redis:get(TAGEER.."pinnedmsg" .. msg.chat_id_) and redis:get(TAGEER.."bot:pin:mute" .. msg.chat_id_) then
unpinmsg(msg.chat_id_)
local pin_id = redis:get(TAGEER.."pinnedmsg" .. msg.chat_id_)
pinmsg(msg.chat_id_, pin_id, 0)
end
if not redis:get(TAGEER.."Resetdatapost" .. msg.chat_id_) then
redis:del(TAGEER.."Gp:Post" .. msg.chat_id_)
redis:setex(TAGEER.."Resetdatapost" .. msg.chat_id_, 12 * hour, true)
end
----------------------com viewget -----------------------------------------------------
if redis:get(TAGEER.."bot:viewget" .. msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, "✯↓ حدث خطا حاول مره اخرى", 1, "md")
redis:del(TAGEER.."bot:viewget" .. msg.sender_user_id_)
else
send(msg.chat_id_, msg.id_, 1, "📛↓ عدد المشاهدات \n: " .. msg.views_ .. "", 1, "md")
redis:del(TAGEER.."bot:viewget" .. msg.sender_user_id_)
end end
if redis:get(TAGEER.."bot:viewmsg") == "On" then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_) then
...
redis:del(TAGEER..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
setphoto(msg.chat_id_, photo_id)
end
------------------------------------------------------------------------------------------------
if redis:get(TAGEER.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local gps = redis:scard(TAGEER.."bot:groups") or 0
local gpss = redis:smembers(TAGEER.."bot:groups") or 0
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:del(TAGEER.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
if redis:get(TAGEER.."anti-flood:" .. msg.chat_id_) then
redis:setex(TAGEER..pm, TIME_CHECK, msgs + 1)
end
--«««««««««««««««««««««««« Developer By TAGEER »»»»»»»»»»»»»»»»»»»»»»»»»»»--
if msg.content_.caption_ then
...
if redis:get(TAGEER.."bot:duplipost:mute" .. msg.chat_id_) then
if redis:sismember(TAGEER.."Gp:Post" .. msg.chat_id_, post) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:sadd(TAGEER.."Gp:Post" .. msg.chat_id_, post)
end
end
if redis:get(TAGEER.."post:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.forward_info_ and redis:get(TAGEER.."bot:forward:mute" .. msg.chat_id_) and (msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost") then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(TAGEER.."bot:photo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:strict" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
end
if msg.content_.caption_:match("@") then
if redis:get(TAGEER.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityMention" and redis:get(TAGEER.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("#") then
if redis:get(TAGEER.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityHashtag" and redis:get(TAGEER.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and (msg.content_.entities_[0].ID == "MessageEntityTextUrl" or msg.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(TAGEER.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.web_page_ and redis:get(TAGEER.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\128-\191]") and redis:get(TAGEER.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if (msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]")) and redis:get(TAGEER.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."markdown:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."Game:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(TAGEER.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:document:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\121-\191]") and redis:get(TAGEER.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:inline:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."clerk") == "On" or is_admin(msg.sender_user_id_) then
...
if redis:get(TAGEER.."bot:sticker:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:tgservice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(TAGEER.."bot:member:lock" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
...
for i = 0, #list do
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(TAGEER.."bot:bots:mute" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(TAGEER.."bot:botskick" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
local user_info_ = redis:get(TAGEER..'user:Name' .. msg.sender_user_id_)
local UserTAGEER = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "👨‍✈️┊ العضو » (["..UserTAGEER.."])\n🚫┊ الايدي » (*"..msg.sender_user_id_.."*)\n🗯┊ قام بأضافه بوت في المجموعه\n📌┊ تم طرد البوت مع العضو \n✓", 1, "md")
end 
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(TAGEER.."keed_bots"..msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
redis:sadd(TAGEER..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. TAGEER_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(TAGEER..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(TAGEER.."bot:member:lock" .. msg.chat_id_) and not is_vipmem(msg.content_.members_[0].id_, msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
...
if redis:get(TAGEER.."bot:contact:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:music:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:voice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:location:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:video:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:selfvideo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:gifs:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_ then
check_filter_words(msg, msg.content_.caption_)
if redis:get(TAGEER.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:setex(TAGEER.."bot:editid" .. msg.id_, day, msg.content_.text_)
if redis:get(TAGEER.."anti-flood:" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
redis:setex(TAGEER..pm, TIME_CHECK, msgs + 1)
end
---------------------rem cam filters---------------------------------------------------------------------
if redis:get(TAGEER.."Filtering:" .. msg.sender_user_id_) then
local chat = redis:get(TAGEER.."Filtering:" .. msg.sender_user_id_)
...
---------------save name bot-----------------------------
if redis:get(TAGEER..'botts:namess'..msg.sender_user_id_) then
redis:del(TAGEER..'botts:namess'..msg.sender_user_id_)
local NAME_BOT = msg.content_.text_:match("(.*)")
redis:set(TAGEER..'TAGEERnams',NAME_BOT)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع اسم البوت 🍃",1, 'html')
return false
end
------------------------save cam link-----------------------------------
if redis:get(TAGEER.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
local glink = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
local hash = "bot:group:link" .. msg.chat_id_
redis:set(TAGEER..hash, glink)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم  حفظ الرابط 🎈", 1, "md")
redis:del(TAGEER.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
--------------------------------------------------------------------------
if redis:get(TAGEER.."gettextsec" .. msg.sender_user_id_) then
local clerktext = msg.content_.text_
redis:set(TAGEER.."textsec", clerktext)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ الكليشه 🎈", 1, "md")
redis:del(TAGEER.."gettextsec" .. msg.sender_user_id_)
end
---------------------save com rules-----------------------------------------------------------------------------
if redis:get(TAGEER.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local rules = msg.content_.text_
redis:set(TAGEER.."bot:rules" .. msg.chat_id_, rules)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ القوانين 🎋",  1, "md")
redis:del(TAGEER.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
-----------ch join------------------------------------------------------
if redis:get(TAGEER..'Kpch'..msg.sender_user_id_) then
redis:del(TAGEER..'Kpch'..msg.sender_user_id_)
local url , res = https.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/getChatAdministrators?chat_id='..msg.content_.text_..'')
local data = KPJS:decode(url)
if res == 400 then
...
--------------------rem  cam broadcast2--------------------------------------------------------------------------
if redis:get(TAGEER.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
if text:match("^الغاء$") then
send(msg.chat_id_, msg.id_, 1, "🔰- تم الغاء الامر بنجاح🎈 ", 1, "md")
redis:del(TAGEER.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
else
local gps = redis:scard(TAGEER.."bot:groups") or 0
local gpss = redis:smembers(TAGEER.."bot:groups") or 0
local msgs = {
...
if redis:get(TAGEER.."bot:joinbylink:" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
else
end
--------------------set cam sudo  ------------------------------------------
if redis:get(TAGEER.."bot:TAGEER_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
redis:del(TAGEER.."bot:TAGEER_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
local TAGEER_dev = msg.content_.text_:match("(.*)")
redis:set(TAGEER.."TAGEER_dev", TAGEER_dev)
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(TAGEER.."bot:links:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:text:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.text_:match("[\216-\219][\121-\191]") and redis:get(TAGEER.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(TAGEER..hash) then
sens = 400
else
sens = tonumber(redis:get(TAGEER..hash))
end
if redis:get(TAGEER.."bot:spam:mute" .. msg.chat_id_) and string.len(msg.content_.text_) > sens or ctrl_chars > sens or real_digits > sens then
delete_msg(chat, msgs)
print_del_msg("Deleted Because [Lock] [Spam] ")
end
end
if (msg.content_.text_:match("[A-Z]") or msg.content_.text_:match("[a-z]")) and redis:get(TAGEER.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(TAGEER.."bot:cmds" .. msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
...
-------------------------------leave groups----------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' غادر$')) then
...
redis:set(TAGEER.."TAGEER_O" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(TAGEER.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_TAGEER = tonumber(redis:get(TAGEER.."TAGEER_O" .. msg.chat_id_)) 
if msgs == get_TAGEER and not redis:get(TAGEER.."lock_addd"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ هذا العضو اثبت تفاعله\n🎟┊تم رفعه مدير في المجموعه\n✓",  1, "md")
redis:sadd(TAGEER..'bot:momod:'..msg.chat_id_, msg.sender_user_id_)
end
--------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "⚜️┊ تم حفظ العدد *"..uuuu[2].."*\n💬┊ سيتم رفع العضو مميز\n🚫┊اذا اثبت تفاعله\n✓",1, 'md')
redis:set(TAGEER.."TAGEER_OO" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(TAGEER.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_TAGEER = tonumber(redis:get(TAGEER.."TAGEER_OO" .. msg.chat_id_)) 
if msgs == get_TAGEER and not redis:get(TAGEER.."lock_adddvip"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "📌┊ هذا العضو اثبت تفاعله\n🌀┊تم رفعه عضو مميز في المجموعه\n✓",  1, "md")
redis:sadd(TAGEER..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
end
------------------------------ADD vipmems BY Reply------------------------------------------------------------------
if text:match('^رفع مميز عام$') and is_KP(msg) and msg.reply_to_message_id_ ~= 0  then
function promote_by_reply(extra, result, success)
local user_info_ = redis:get(TAGEER..'user:Name' .. result.sender_user_id_)
local UserTAGEER = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(TAGEER..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(TAGEER..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم رفعه مميز عام\n✓', 1, 'md')
end end end
...
local hash = 'bot:vipmems:'
if result.id_ then
if redis:sismember(TAGEER..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(TAGEER..hash, result.id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
local user_info_ = redis:get(TAGEER..'user:Name' .. ap[2])
local UserTAGEER = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(TAGEER..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(TAGEER..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(TAGEER..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(TAGEER..hash, result.sender_user_id_)
...
local hash = 'bot:vipmems:'
if result.id_ then
if not redis:sismember(TAGEER..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تنزيله مميز عام سابقا\n‏', 1, 'md')
else
redis:srem(TAGEER..hash, result.id_)
...
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(TAGEER..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(TAGEER..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم حذفه من مميزين العام \n✓‏', 1, 'md')
end end
end
-----------------------------promote_by_reply-------------------------------------------------------
...
function promote_by_username(extra, result, success)
if result.id_ then
if redis:sismember(TAGEER..'bot:vipmem:'..msg.chat_id_, result.id_) then
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(TAGEER..'bot:vipmem:'..msg.chat_id_, result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end
...
local UserTAGEER = user_info_
if user_info_ then
if redis:sismember(TAGEER..'bot:vipmem:'..msg.chat_id_, ap[2]) then
texts = '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(TAGEER..'bot:vipmem:'..msg.chat_id_, ap[2])
texts = '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'')
redis:sadd(TAGEER..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'')
redis:sadd(TAGEER..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'')
redis:sadd(TAGEER..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(TAGEER..hash, result.sender_user_id_)
...
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(TAGEER..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(TAGEER..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
if result.id_ then
redis:sadd(TAGEER..'bot:admins:', result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم اضافه مدير في البوت \n✓‏'
else
...
redis:srem(TAGEER..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم تنزيله من المطورين \n✓‏', 1, 'md')
table.remove(_config.Sudo_Users, getindex(_config.Sudo_Users, mohmad))
save_on_config()
load_config()
end
end
...
local UserTAGEER = user_info_
if user_info_ then
local k1 = not redis:sismember(TAGEER..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(TAGEER..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(TAGEER..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(TAGEER..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
redis:srem(TAGEER..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
redis:srem(TAGEER..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(TAGEER..'bot:owners:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(TAGEER..'bot:momod:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(TAGEER..'bot:vipmem:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(TAGEER..'bot:owners:'..msg.chat_id_, result.id_)
redis:srem(TAGEER..'bot:momod:'..msg.chat_id_, result.id_)
redis:srem(TAGEER..'bot:vipmem:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تجريد الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserTAGEER = user_info_
if user_info_ then
local k1 = not redis:sismember(TAGEER..'bot:owners:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(TAGEER..'bot:momod:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(TAGEER..'bot:vipmem:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(TAGEER..'bot:owners:'..msg.chat_id_, ap[2])
redis:srem(TAGEER..'bot:momod:'..msg.chat_id_, ap[2])
redis:srem(TAGEER..'bot:vipmem:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم تجريد الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserTAGEER = user_info_
if user_info_ then
local k1 = not redis:sismember(TAGEER..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(TAGEER..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(TAGEER..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ لا توجد لديه قيود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(TAGEER..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
redis:srem(TAGEER..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
redis:srem(TAGEER..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(TAGEER..'bot:keed:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(TAGEER..'bot:muted:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(TAGEER..'bot:banned:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه قيود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(TAGEER..'bot:keed:'..msg.chat_id_, result.id_)
redis:srem(TAGEER..'bot:muted:'..msg.chat_id_, result.id_)
redis:srem(TAGEER..'bot:banned:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
local UserTAGEER = user_info_
if user_info_ then
local k1 = not redis:sismember(TAGEER..'bot:keed:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(TAGEER..'bot:muted:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(TAGEER..'bot:banned:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ لا توجد لديه قيود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(TAGEER..'bot:keed:'..msg.chat_id_, ap[2])
redis:srem(TAGEER..'bot:muted:'..msg.chat_id_, ap[2])
redis:srem(TAGEER..'bot:banned:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserTAGEER..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
if redis:get(TAGEER.."lock_GEM"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ اللعبه معطله\n ‏ ", 1, "md")
return false end
local user_info_ = redis:get(TAGEER.."user:Name" .. msg.sender_user_id_)
local UserTAGEER = user_info_
if user_info_ then
redis:set(TAGEER..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'','kk')
send(msg.chat_id_, 0, 1, '👨‍✈️» اهلا ['..UserTAGEER..'] \n™️» في لعبه السرعه\n⚠️» ارسل ( الاسرع ) للعب\n✓',1, 'md')
return false end end
if text == 'الاسرع'  and is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(TAGEER..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'') then
local keep1 = {'س م ى و','ى-س-م-و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep1[math.random(#keep1)]..')*',1, 'md')
redis:set(TAGEER..'kk11'..msg.chat_id_..'','kkk')
return false end
if text then
local TAGEER1 = redis:get(TAGEER..'kk11'..msg.chat_id_..'')
if TAGEER1 == 'kkk' then
if text == 'موسى' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'kk11'..msg.chat_id_..'')
sleep(1.5)
local ooo = {'ا-ل-ي-ي-ب','ي ي ا ل ب'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ooo[math.random(#ooo)]..')*',1, 'md')
redis:set(TAGEER..'kk111'..msg.chat_id_..'','kkkk')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'kk111'..msg.chat_id_..'')
if TAGEER1 == 'kkkk' then
if text == 'ليبيا' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'kk111'..msg.chat_id_..'')
sleep(1.5) 
local keep = {'ر ط ا ي ه','ا-ي-ط-ر-ه'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep[math.random(#keep)]..')*',1, 'md')
redis:set(TAGEER..'kk1111'..msg.chat_id_..'','mm')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'kk1111'..msg.chat_id_..'')
if TAGEER1 == 'mm' then
if text == 'طياره' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'kk1111'..msg.chat_id_..'')
sleep(1.5)
local keep77 = {'😛😛😛😝😛😛😛','😜😝😜😜😜😜😜'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep77[math.random(#keep77)]..')*',1, 'md')
redis:set(TAGEER..'w1'..msg.chat_id_..'','q1')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w1'..msg.chat_id_..'')
if TAGEER1 == 'q1' then
if text == '😝' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w1'..msg.chat_id_..'')
sleep(1.5)
local keep2 = {'ش-ر-ط-ي','ش,ط,ر,ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep2[math.random(#keep2)]..')*',1, 'md')
redis:set(TAGEER..'w2'..msg.chat_id_..'','q2')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w2'..msg.chat_id_..'')
if TAGEER1 == 'q2' then
if text == 'شرطي' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w2'..msg.chat_id_..'')
sleep(1.5)
local keep3 = {'😃😃😄😃😃😃'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep3[math.random(#keep3)]..')*',1, 'md')
redis:set(TAGEER..'w9'..msg.chat_id_..'','q9')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w9'..msg.chat_id_..'')
if TAGEER1 == 'q9' then
if text == '😄' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w9'..msg.chat_id_..'')
sleep(1.5)
local keep4 = {'ر ك و ا ي','ر ا ي ك و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep4[math.random(#keep4)]..')*',1, 'md')
redis:set(TAGEER..'w4'..msg.chat_id_..'','q4')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w4'..msg.chat_id_..'')
if TAGEER1 == 'q4' then
if text == 'كوريا' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w4'..msg.chat_id_..'')
sleep(1.5)
local keep5 = {'ك - م - ا - ل','ل ك ا م'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep5[math.random(#keep5)]..')*',1, 'md')
redis:set(TAGEER..'w5'..msg.chat_id_..'','q5')
return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w5'..msg.chat_id_..'')
if TAGEER1 == 'q5' then
if text == 'ملاك' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w5'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n(ك , ش , ب , ا)',1, 'md')
redis:set(TAGEER..'w6'..msg.chat_id_..'','q6')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w6'..msg.chat_id_..'')
if TAGEER1 == 'q6' then
if text == 'شباك' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w6'..msg.chat_id_..'')
sleep(1.5)
local keep6 = {'ل-ج-م-ي','ل ي م ج'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep6[math.random(#keep6)]..')*',1, 'md')
redis:set(TAGEER..'w7'..msg.chat_id_..'','q7')
return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w7'..msg.chat_id_..'')
if TAGEER1 == 'q7' then
if text == 'جميل' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w7'..msg.chat_id_..'')
sleep(1.5)
local keep7 = {'و ك ه ه','ه ك ه و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep7[math.random(#keep7)]..')*',1, 'md')
redis:set(TAGEER..'w8'..msg.chat_id_..'','q8')
return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w8'..msg.chat_id_..'')
if TAGEER1 == 'q8' then
if text == 'كهوه' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w8'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n( ن ،ص، و، ك، د)',1, 'md')
redis:set(TAGEER..'w9o'..msg.chat_id_..'','q9o')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w9o'..msg.chat_id_..'')
if TAGEER1 == 'q9o' then
if text == 'صندوك' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w9o'..msg.chat_id_..'')
sleep(1.5)
local keep8 = {'ض ر م ي','م ض ر ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep8[math.random(#keep8)]..')*',1, 'md')
redis:set(TAGEER..'w00'..msg.chat_id_..'','q00')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'w00'..msg.chat_id_..'')
if TAGEER1 == 'q00' then
if text == 'مريض' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'w00'..msg.chat_id_..'')
sleep(1.5)
local keep9 = {'ي- ط -ر -ق -ن -ب -ا',' ب ي ق ا ط ر ا ن','ب,ي,ا,,ق,ر,ن,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep9[math.random(#keep9)]..')*',1, 'md')
redis:set(TAGEER..'a15'..msg.chat_id_..'','s15')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'a15'..msg.chat_id_..'')
if TAGEER1 == 's15' then
if text == 'قرنابيط' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'a15'..msg.chat_id_..'')
sleep(1.5)
local keep00 = {'😔😔😔😔😔😔😔☺️😔😔😔😔😔'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep00[math.random(#keep00)]..')*',1, 'md')
redis:set(TAGEER..'a26'..msg.chat_id_..'','s26')
return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'a26'..msg.chat_id_..'')
if TAGEER1 == 's26' then
if text == '☺️' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'a26'..msg.chat_id_..'')
sleep(1.5)
local keepp = {'ط - م - ي','ي ,م ,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keepp[math.random(#keepp)]..')*',1, 'md')
redis:set(TAGEER..'a99'..msg.chat_id_..'','s99')
 return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'a99'..msg.chat_id_..'')
if TAGEER1 == 's99' then
if text == 'مطي' then
redis:incr(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(TAGEER..'a99'..msg.chat_id_..'')
return false end end end
-----------------------------------------------------------------------------
if text == 'نقاطي' then
send(msg.chat_id_, msg.id_, 1, '💬┊ عدد نقاطك » *('..tonumber(redis:get(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )..')*',1, 'md')
return false end
if text == 'بيع نقاطي' then
local user_info_ = redis:get(TAGEER..'user:Name' .. msg.sender_user_id_)
local UserTAGEER = user_info_
if user_info_ then
local numkep = tonumber(redis:get(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep == 0 then
...
redis:set(TAGEER..'mohmad1'..msg.sender_user_id_..''..msg.chat_id_..'','mohmad2')
send(msg.chat_id_, msg.id_, 1, keep,1, 'md')
return false end end end
if text then
local TAGEER1 = redis:get(TAGEER..'mohmad1'..msg.sender_user_id_..''..msg.chat_id_..'')
if TAGEER1 == 'mohmad2' then
if text == '1' then
if redis:sismember(TAGEER..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'⚠️┊ عذرا انت مرفوع مميز سابقا\n✓', 1, 'md')
redis:del(TAGEER..'mohmad1'..msg.sender_user_id_..''..msg.chat_id_..'')
else
local numkep = tonumber(redis:get(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep < tonumber(redis:get(TAGEER.."TAGEER_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '®️┊عذرا نقاطك اقل من '..tonumber(redis:get(TAGEER.."TAGEER_OO0" .. msg.chat_id_) or 100 )..'\n💬┊لا تستطيع تحويلهم لعضو مميز\n✓',1, 'md')
redis:del(TAGEER..'mohmad1'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end
if numkep > tonumber(redis:get(TAGEER.."TAGEER_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '🎰┊مبروك عزيزي\n🎧┊تم رفعك عضو مميز\n ✓',1, 'md')
redis:sadd(TAGEER..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
redis:del(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(TAGEER..'mohmad1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل امر اطردني\n ✓ ", 1, 'md')
redis:set(TAGEER.."lock_kickme"..msg.chat_id_, true)
return false 
end
if text == 'تعطيل اطردني' and is_owner(msg.sender_user_id_, msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تعطيل امر اطردني\n ✓ ", 1, 'md')
redis:del(TAGEER.."lock_kickme"..msg.chat_id_)
return false 
end
-------------------add ch --------------------------
...
redis:setex(TAGEER..'Kpch'..msg.sender_user_id_,300,true)
...
if not redis:get(TAGEER..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, '™️┊ لم يتم وضع القناة\n📌┊ لتعين  القناة ارسل .....\n⚠️┊ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(TAGEER..'Kpch1') then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(TAGEER.."Kpjoin1", true)
return false end end end
...
redis:del(TAGEER.."dellmssg"..msg.chat_id_)
end
----------------ADD DEL MSG------------------------------------------------------------
...
redis:set(TAGEER.."dellmssg"..msg.chat_id_, true)
end
----------add - rem bot free----------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل البوت خدمي\n ✓ ", 1, 'md')
redis:set(TAGEER.."bot:free", true)
end end
...
local UserTAGEER = user_info_
if user_info_ then
local start = redis:get(TAGEER.."startbot")
if start then
keep = [[
[]]..start..[[]
]]
...
💠┊مرحبا انا بوت اسمي *(]]..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..[[)🗼*
...
🔱┊*مطور البوت* » ( []] .. UserTAGEER .. [[] )‏
‏
‏]]
send(msg.chat_id_, msg.id_, 1, TAGEER, 1, 'md')
end end end
-----------------------get start------------------------------------------------------------------------
...
local Kpch2 = redis:get(TAGEER.."Kpch1")
if Kpch2 then
...
text = "🌀┊  مدراء البوت :\n*≖≖≖≖≖≖≖≖≖≖≖≖*\n"
for k, v in pairs(list) do
if redis:get(TAGEER.."SudoNumberGp" .. v) then
gps = tonumber(redis:get(TAGEER.."SudoNumberGp" .. v))
else
gps = 0
end
local user_info = redis:get(TAGEER.."user:Name" .. v)
...
local time = a[2] * day
redis:setex(TAGEER.."bot:charge:"..msg.chat_id_,time,true)
redis:set(TAGEER.."bot:enable:"..msg.chat_id_,true)
end
------------------charge-----------------------------------------------------------------------------
if text:match("^فحص الشحن") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local ex = redis:ttl(TAGEER.."bot:charge:"..msg.chat_id_)
if ex == -1 then
...
local ex = redis:ttl(TAGEER.."bot:charge:"..txt[2])
if ex == -1 then
...
if not redis:get(TAGEER.."bot:free") and not is_sudo(msg) then
send(msg.chat_id_, msg.id_, 1, '🚫┊* للمطورين فقط* ❗️', 1, 'md')
return false end
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(TAGEER..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
if result.members_[i].status_.ID == "ChatMemberStatusCreator" then
owner_id = admins[i].user_id_
redis:sadd(TAGEER.."bot:owners:"..msg.chat_id_,owner_id)
end
end
end
getChannelMembers(msg.chat_id_, 0, 'Administrators', 200, promote_admin)
redis:set(TAGEER.."test:group"..msg.chat_id_,'krar')
if redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) then
...
if not redis:get(TAGEER.."bot:group:link"..msg.chat_id_) then
local getlink = 'https://api.telegram.org/bot'..TAGEER_TOKEN..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = KPJS:decode(req)
if link.ok == true then 
redis:set(TAGEER.."bot:group:link"..msg.chat_id_,link.result)
end
end
local lik_1 = redis:get(TAGEER.."bot:group:link"..msg.chat_id_)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(Kp_Owner,0, 1,"- *تم اضافه مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ مدير عام عام ≈ [" .. owner .. "]\n🔰┊ ["..title_name(msg.chat_id_).."]("..(link or "t.me/TAGEER_ch")..")\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " ..result.first_name_.. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n✓", 1, "md")
redis:set(TAGEER.."bot:enable:" .. msg.chat_id_, true)
redis:setex(TAGEER.."bot:charge:" .. msg.chat_id_, 9999 * day, true)
redis:sadd(TAGEER.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
end
tdcli_function ({
ID = "GetChannelFull",
...
redis:set(TAGEER.."bot:charge:"..txt[2],true)
...
local txt = { string.match(text, "^(تفعيل)$")}
redis:del(TAGEER..'lock:add'..msg.chat_id_)
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(TAGEER..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
...
redis:set(TAGEER..'lock:add'..msg.chat_id_,true)
if not redis:get(TAGEER.."bot:enable:" .. msg.chat_id_) then
...
redis:del(TAGEER.."bot:enable:" .. msg.chat_id_)
redis:srem(TAGEER.."bot:groups", msg.chat_id_)
redis:del(TAGEER.."test:group"..msg.chat_id_)
redis:srem(TAGEER.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
...
send(v,0, 1,"- *تم تعطيل مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ مدير عام عام ≈ " .. owner .. "\n🔰┊ الرابط ≈ [اضغط هنا](" .. (redis:get(TAGEER.."bot:group:link"..msg.chat_id_) or "https://t.me/TAGEER_ch") .. ")\n🔅┊* اسم المجموعه* »\n👨🏼┊ﮧ "..title_name(msg.chat_id_).."\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " .. fname .. " " .. lname .. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n‏", 1, "md")
end
...
local gp = txt[3]
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم تفعيل المجموعه \n(*"..txt[2].."*) يوم 🍃", 1, 'html')
redis:sadd(TAGEER..'sudo:data:'..sudo, gp)
end
-------------------REEM----------------------------------------------------------------------------
...
local send_to_bot_owner = function(extra, result)
redis:del(TAGEER.."bot:enable:" .. gp[2])
redis:del(TAGEER.."bot:charge:" .. gp[2])
local v = tonumber(Kp_Owner)
local fname = result.first_name_ or ""
local lname = result.last_name_ or ""
local username = "@" .. result.username_ or ""
...
redis:srem(TAGEER.."sudo:data:" .. msg.sender_user_id_, gp[2])
redis:srem(TAGEER.."bot:groups", gp[2])
end
getUser(msg.sender_user_id_, send_to_bot_owner)
end
else
...
local num_keep = (tonumber(redis:get(TAGEER.."incr_msg"..result.id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(TAGEER.."msgs:"..result.id_..":"..msg.chat_id_) or  0))
local Kpcontact = (tonumber(redis:get(TAGEER.."kpaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
...
mohmad_info2 = "🎟┊ ايديه  » `" .. result.id_ .. "`\n©️┊ معرفه »  [@" .. ap[2] .. "]\n👤┊ جهاته  »   "..Kpcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. t .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
elseif not result.id_ then
mohmad_info2 = "🌀┊ لا يوجد عضو بهذا المعرف"
end
send(msg.chat_id_, msg.id_, 1, mohmad_info2, 1, "md")
end
...
local UserTAGEER = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "•اهلا ~ ["..UserTAGEER.."]\n🎧» ايديك  (`"..msg.sender_user_id_.."`)\n✓", 1, "md")
return false end end
-------------------RETBA-------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local num_keep = tonumber(redis:get(TAGEER..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
local msgs = tonumber(redis:get(TAGEER.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local Kpcontact = (tonumber(redis:get(TAGEER.."kpaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
local mohmad_info = "🎟┊ ايديك  » " .. msg.sender_user_id_ .. "\n©️┊ معرفك »  " .. username .. "\n👤┊ جهاتك  »   "..Kpcontact.."\n🌄┊ صورك  »   "..result.total_count_.."\n🔆┊ نقاطك  »   "..num_keep.."\n🌐┊ تفاعلك »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائلك » " .. msgs .. "\n📌┊ موقعك » " .. tar .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
local mohmad_info2 = "🎟┊ ايديك  » `" .. msg.sender_user_id_ .. "`\n©️┊ معرفك »  [" .. username .. "]\n👤┊ جهاتك  »   "..Kpcontact.."\n🌄┊ صورك  »   "..result.total_count_.."\n🔆┊ نقاطك  »   "..num_keep.."\n🌐┊ تفاعلك »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائلك » " .. msgs .. "\n📌┊ موقعك » " .. tar .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"

if redis:get(TAGEER.."getidstatus" .. msg.chat_id_) == "Photo" then
if result.photos_[0] then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_, mohmad_info)
else
send(msg.chat_id_, msg.id_, 1, "انت لا تمتلك صوره لحسابك🎈‏\n"..mohmad_info2.."", 1, "md")
end
end
if redis:get(TAGEER.."getidstatus" .. msg.chat_id_) == "Simple" then
send(msg.chat_id_, msg.id_, 1, mohmad_info2, 1, "md")
end
if not redis:get(TAGEER.."getidstatus" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, mohmad_info2, 1, "md")
 end 
 end
tdcli_function({
...
--------------------Kp concat add----------------------------------------
if text == 'جهاتي' then
send(msg.chat_id_, msg.id_, 1, '🔰┊ عدد جهاتك ≈ * '..(tonumber(redis:get(TAGEER..'kpaddcon'..msg.chat_id_..':'..msg.sender_user_id_) or 0))..' *',1,'md')
end
--------------------ID BY REPLY------------------------------------------
if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
function iD_reP(extra, result, success)
local num_keep = (tonumber(redis:get(TAGEER.."incr_msg"..result.sender_user_id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(TAGEER.."msgs:"..result.sender_user_id_..":"..msg.chat_id_) or 0 ))
local Kpcontact = (tonumber(redis:get(TAGEER.."kpaddcon"..msg.chat_id_..":"..result.sender_user_id_) or 0))
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(Kp_Owner) then
TAGEER3 = "مطور الاساسي 🌿"
elseif is_sudoid(result.sender_user_id_) then
TAGEER3 = "المطور 🐾"
elseif is_admin(result.sender_user_id_) then
TAGEER3 = "مدير في البوت"
elseif is_vipmems(result.sender_user_id_) then
TAGEER3 = "مميز عام 🌿"
elseif is_monshi(result.sender_user_id_, msg.chat_id_) then
TAGEER3 = "منشىء الكروب 🐾"
elseif is_owner(result.sender_user_id_, msg.chat_id_) then
TAGEER3 = "مدير عام عام 🌿"
elseif is_momod(result.sender_user_id_, msg.chat_id_) then
TAGEER3 = "مدير المجموعه🐾"
...
local TAGEER_info = "🎟┊ ايديه  » `" .. result.sender_user_id_ .. "`\n👤┊ جهاته  »   "..Kpcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. KP_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. TAGEER3 .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
send(msg.chat_id_, result.id_, 1, TAGEER_info, 1, "md")
end
...
redis:setex(TAGEER..'botts:namess'..msg.sender_user_id_,698,true)
...
if redis:get(TAGEER.."getpro:" .. msg.chat_id_) == "Active" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
redis:set(TAGEER.."getpro:" .. msg.chat_id_, "Active")
end end end
...
if redis:get(TAGEER.."getpro:" .. msg.chat_id_) == "Deactive" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
redis:set(TAGEER.."getpro:" .. msg.chat_id_, "Deactive")
end end end
---------------------------------------------------------------------
if text:match("^مسح الصوره") and is_momod(msg.sender_user_id_, msg.chat_id_)  then
https.request('https://api.telegram.org/bot'..TAGEER_TOKEN..'/deleteChatPhoto?chat_id='..msg.chat_id_)
...
if not is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(TAGEER.."getpro:" .. msg.chat_id_) == "Deactive" then
...
redis:set(TAGEER..'flood:time:'..msg.chat_id_,floodt[2])
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التكرار بالطرد\n ✓ ", 1, 'md')
redis:set(TAGEER.."floodstatus" .. msg.chat_id_, "Kicked")
end
end
if status[2] == "بالمسح" then
if redis:get(TAGEER.."floodstatus" .. msg.chat_id_) == "DelMsg" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
redis:set(TAGEER.."floodstatus" .. msg.chat_id_, "DelMsg")
end
end
end
-------------------SET warn-----------------------------------------
...
redis:set(TAGEER.."warn:max:" .. msg.chat_id_, warnmax[2])
end
end
---------------------warnstatus----------------------------------------------------------
...
if redis:get(TAGEER.."warnstatus" .. msg.chat_id_) == "Muteuser" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
redis:set(TAGEER.."warnstatus" .. msg.chat_id_, "Muteuser")
end   end
if status[2] == "بالطرد" then
if redis:get(TAGEER.."warnstatus" .. msg.chat_id_) == "Remove" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
redis:set(TAGEER.."warnstatus" .. msg.chat_id_, "Remove")
end end end
-----------------------getidstatus-----------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(TAGEER.."getidstatus" .. msg.chat_id_, "Photo")
end end  end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تعطيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(TAGEER.."getidstatus" .. msg.chat_id_, "Simple")
end end end
-------------------autoleave-----------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تفعيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(TAGEER.."autoleave", "On")
end end end
if is_sudo(msg) and text:match("^تعطيل (.*)$") then
local status = { string.match(text, "^(تعطيل) (.*)$")}
if status[2] == "الخروج التلقائي" then
if redis:get(TAGEER.."autoleave") == "Off" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(TAGEER.."autoleave", "Off")
end   end  end
-----------------------------------------------------------
...
redis:setex(TAGEER.."bot:TAGEER_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 370, true)
end end
...
local TAGEER_dev = redis:get(TAGEER.."TAGEER_dev")
if TAGEER_dev then
send(msg.chat_id_, msg.id_, 1, "["..TAGEER_dev.."]", 1, "md")
else
local mohmad = [[
...
⚜️» مطور البوت » []]..UserTAGEER..[[]
〰
]]
send(msg.chat_id_, msg.id_, 1, mohmad, 1, "md") 
end
end
end
getUser(msg.sender_user_id_, get_me)
...
redis:setex(TAGEER.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 1200, true)
end
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^حذف الرابط$"))  then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم حذف الرابط بنجاح\n ✓ ", 1, 'md')
redis:del(TAGEER.."bot:group:link" .. msg.chat_id_)
end
if text:match("^الرابط$") then
local link = redis:get(TAGEER.."bot:group:link" .. msg.chat_id_)
if link then
send(msg.chat_id_, msg.id_, 1, "📬¦ رابط المجموعه :\n" .. link .. "", 1, "md")
else
...
redis:setex(TAGEER.."Filtering:" .. msg.sender_user_id_, 80, msg.chat_id_)
end
----------------------------------------------------------------------------------
...
local hash = "bot:filters:" .. msg.chat_id_
if not redis:hget(TAGEER..hash, name) then
send(msg.chat_id_, msg.id_, 1, "🌀┊  الكلمه〖  " .. name .. "  〗\n👥┊  تم الغاء منعها📍", 1, "md")
redis:hdel(TAGEER..hash, name)
end end
-------------------USERS----------------------------------------------------
if text == 'المشتركين' or text == 'عدد المشتركين' and is_KP(msg) then
local users = redis:scard(TAGEER.."bot:userss")
local botnamess = redis:get(TAGEER.."TAGEERnams") or "كيبر"
...
local get_me = function(extra, result)
local num = (redis:scard(TAGEER.."bot:groups"))
local list = redis:smembers(TAGEER.."bot:groups")
local text = "~ All_Groups_Bots \n\n"
for k,v in pairs(list) do
local GroupsOwner = redis:scard(TAGEER.."bot:owners:"..v) or 0
local GroupsMod = redis:scard(TAGEER.."bot:momod:"..v) or 0
local Groupslink = redis:get(TAGEER.."bot:group:link" ..v)
if result.first_name_ then
if #result.first_name_ < 35 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
file:write(text)
file:close()
local token_files = 'https://api.telegram.org/bot' .. TAGEER_TOKEN .. '/sendDocument'
...
local botnamess = redis:get(TAGEER.."TAGEERnams") or "كيبر"
local kpSudos1 = redis:scard(TAGEER.."Bot:KpSudos") or "0"
local kpAdmins = redis:scard(TAGEER.."Bot:Admins") or "0"
local kpgban = redis:scard(TAGEER.."bot:gban:") or "0"
local kpvipmems = redis:scard(TAGEER.."bot:vipmems:") or "0"
local kpgps = redis:scard(TAGEER.."bot:groups") or "0"
local kpusers = redis:scard(TAGEER.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البوت ≈*\n*ﮧ -------------------»*\n👨🏼┊اسم البوت ≈ *" .. botnamess .. "*\n🃏┊عدد المطورين ≈ *"..kpSudos1.."*\n🔰┊عدد المدراء ≈ *"..kpAdmins.."*\n🔚┊عدد الكروبات ≈ * "..kpgps.."*\n👪┊عدد المشتركين خاص ≈ *" .. kpusers .. "*\n🔔┊عدد المحظورين عام ≈ *"..kpgban.."*\n🔘┊عدد المميزين عام ≈ * "..kpvipmems.."*\n🗯┊قناة السورس ≈ [TAGEER](t.me/TAGEER_ch)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
if text:match("^المجموعات$") or text:match("^الكروبات$") and is_admin(msg.sender_user_id_, msg.chat_id_) then
local gps = redis:scard(TAGEER.."bot:groups")
local users = redis:scard(TAGEER.."bot:userss")
...
function cb(a,b,c)
local text = b.content_.text_
local users = redis:scard(TAGEER.."bot:userss") or 0
local userss = redis:smembers(TAGEER.."bot:userss") or 0
local msgs = {
[0] = id
}
for i = 1, #userss do
...
if redis:get(TAGEER.."ffosh"..msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = { [0] = id}
local chat = msg.chat_id_
delete_msg(chat, msgs)
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ الرتبه » (*"..TAGEER3.."*) 🍃\n‏" , 1, "md")
end
...
`git clone https://github.com/alqaser/TAGEER.git && cd TAGEER && chmod +x TAGEER && ./TAGEER`
...
`killall screen && cd TAGEER && screen ./TAGEER`

💬┊ *مطور السورس *≈ [@rr20r](t.me/rr20r)
💲┊ *قناٌة السوٰرس *≈ [@TAGEER_ch](T.ME/TAGEER_ch)
...
redis:set(TAGEER..'welcome:'..msg.chat_id_,welcome)
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم وضع الترحيب\n ✓ ", 1, 'md')
end
-----------------welcome------------------------------------------
if text:match("^الترحيب$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local wel = redis:get(TAGEER..'welcome:'..msg.chat_id_)
if wel then
...
redis:setex(TAGEER.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
redis:setex(TAGEER.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
send(msg.chat_id_, msg.id_, 1, "🌀┊  ارسل لي الرساله الان📲\n🚫┊ للخروج ارسل لي الغاء 📍 ", 1, "md")
end end end
----------------- info bot --------------------------------------------------------------
...
local gps = redis:scard(TAGEER.."bot:groups")
local users = redis:scard(TAGEER.."bot:userss")
local allmgs = redis:get(TAGEER.."bot:allmsgs")
if redis:get(TAGEER.."autoleave") == "On" then
autoleaveAR = "مفعل"
elseif redis:get(TAGEER.."autoleave") == "Off" then
autoleaveAR = "معطل"
elseif not redis:get(TAGEER.."autoleave") then
autoleaveAR = "معطل"
end
if redis:get(TAGEER.."joinbylink") == "On" then
joinbylinkAR = "مفعل"
elseif redis:get(TAGEER.."joinbylink") == "Off" then
joinbylinkAR = "معطل"
elseif not redis:get(TAGEER.."joinbylink") then
joinbylinkAR = "معطل"
end
...
Uptime_1 = redis:get(TAGEER.."bot:Uptime")
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local lockTAGEER = {string.match(text, "^(قفل) (.*)$")}
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيه فقط', 1, 'md')
else
if lockTAGEER[2] == "التعديل" then
if not redis:get(TAGEER.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التعديل \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER..'editmsg'..msg.chat_id_,'delmsg')
end
if lockTAGEER[2] == "الاوامر" then
if not redis:get(TAGEER.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الاوامر \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:cmds" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "البوتات" then
if not redis:get(TAGEER.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:bots:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "البوتات بالطرد" then
if not redis:get(TAGEER.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊البوتات بالطرد مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:botskick" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "التكرار" then
if not redis:get(TAGEER.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التكرار \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."anti-flood:" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "التثبيت" then
if not redis:get(TAGEER.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التثبيت \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:pin:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الكل" then
if not redis:get(TAGEER.."bot:muteall" .. msg.chat_id_) then
...
redis:set(TAGEER.."bot:bots:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."anti-flood:" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:photo:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:video:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:selfvideo:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:document:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."markdown:lock" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:gifs:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:music:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:voice:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."tags:lock" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:contact:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:tgservice:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."bot:forward:mute" .. msg.chat_id_, true)
redis:set(TAGEER.."ffosh"..msg.chat_id_, true)
end
if lockTAGEER[2] == "الدردشه" then
if not redis:get(TAGEER.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الدردشه \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:text:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الصور" then
if not redis:get(TAGEER.."bot:photo:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "المشاركه" then
if not redis:get(TAGEER.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل المشاركه \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:duplipost:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الكلايش" then
if not redis:get(TAGEER.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الكلايش \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:spam:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الفيديو" then
if not redis:get(TAGEER.."bot:video:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "السيلفي" then
if not redis:get(TAGEER.."bot:selfvideo:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الفايلات" then
if not redis:get(TAGEER.."bot:document:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الالعاب" then
if not redis:get(TAGEER.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الالعاب \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."Game:lock" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الانلاين" then
if not redis:get(TAGEER.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانلاين \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:inline:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "البوست" then
if not redis:get(TAGEER.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوست \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."post:lock" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الماركدون" then
if not redis:get(TAGEER.."markdown:lock" .. msg.chat_id_) then
...
if lockTAGEER[2] == "المتحركه" then
if not redis:get(TAGEER.."bot:gifs:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الاغاني" then
if not redis:get(TAGEER.."bot:music:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الصوت" then
if not redis:get(TAGEER.."bot:voice:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الروابط" then
if not redis:get(TAGEER.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الروابط \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:links:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "المواقع" then
if not redis:get(TAGEER.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل المواقع \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:location:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "التاك" then
if not redis:get(TAGEER.."tags:lock" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الهاشتاك" then
if not redis:get(TAGEER.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الهاشتاك \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:hashtag:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الجهات" then
if not redis:get(TAGEER.."bot:contact:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الويب" then
if not redis:get(TAGEER.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الويب \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:webpage:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "العربيه" then
if not redis:get(TAGEER.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل العربيه \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:arabic:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الانظمام" then
if not redis:get(TAGEER.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانظمام \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:member:lock" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الانكليزيه" then
if not redis:get(TAGEER.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانكليزي \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:english:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "الاشعارات" then
if not redis:get(TAGEER.."bot:tgservice:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الملصقات" then
if not redis:get(TAGEER.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الملصقات \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:set(TAGEER.."bot:sticker:mute" .. msg.chat_id_, true)
end
if lockTAGEER[2] == "التوجيه" then
if not redis:get(TAGEER.."bot:forward:mute" .. msg.chat_id_) then
...
if lockTAGEER[2] == "الفشار" then
if not redis:get(TAGEER.."ffosh" .. msg.chat_id_) then
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local UNTAGEER = { string.match(text, "^(فتح) (.*)$") }
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيه فقط', 1, 'md')
else
if UNTAGEER[2] == "التعديل" then
if redis:get(TAGEER.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التعديل \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
redis:del(TAGEER.."editmsg" .. msg.chat_id_)
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
end
if UNTAGEER[2] == "الاوامر" then
if redis:get(TAGEER.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الاوامر \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:cmds" .. msg.chat_id_)
end
if UNTAGEER[2] == "البوتات" then
if redis:get(TAGEER.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:bots:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "البوتات بالطرد" then
if redis:get(TAGEER.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات بالطرد مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:botskick" .. msg.chat_id_)
end
if UNTAGEER[2] == "التكرار" then
if redis:get(TAGEER.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التكرار \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."anti-flood:" .. msg.chat_id_)
end
if UNTAGEER[2] == "التثبيت" then
if redis:get(TAGEER.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التثبيت \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:pin:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الكل" then
if redis:get(TAGEER.."bot:muteall" .. msg.chat_id_) then
...
redis:del(TAGEER.."bot:bots:mute" .. msg.chat_id_)
redis:del(TAGEER.."anti-flood:" .. msg.chat_id_)
redis:del(TAGEER.."bot:photo:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:video:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:selfvideo:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:document:mute" .. msg.chat_id_)
redis:del(TAGEER.."markdown:lock" .. msg.chat_id_)
redis:del(TAGEER.."bot:gifs:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:music:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:voice:mute" .. msg.chat_id_)
redis:del(TAGEER.."tags:lock" .. msg.chat_id_)
redis:del(TAGEER.."bot:contact:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:tgservice:mute" .. msg.chat_id_)
redis:del(TAGEER.."bot:forward:mute" .. msg.chat_id_)
redis:del(TAGEER.."ffosh"..msg.chat_id_)
end
if UNTAGEER[2] == "الدردشه" then
if redis:get(TAGEER.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الدردشه \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:text:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الصور" then
if redis:get(TAGEER.."bot:photo:mute" .. msg.chat_id_) then
...
if UNTAGEER[2] == "المشاركه" then
if redis:get(TAGEER.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح المشاركه \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:duplipost:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الكلايش" then
if redis:get(TAGEER.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الكلايش \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:spam:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الفيديو" then
if redis:get(TAGEER.."bot:video:mute" .. msg.chat_id_) then
...
if UNTAGEER[2] == "الالعاب" then
if redis:get(TAGEER.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الالعاب \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."Game:lock" .. msg.chat_id_)
end
if UNTAGEER[2] == "الانلاين" then
if redis:get(TAGEER.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانلاين \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:inline:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "البوست" then
if redis:get(TAGEER.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوست \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")            
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."post:lock" .. msg.chat_id_)
end
if UNTAGEER[2] == "الماركدون" then
if redis:get(TAGEER.."markdown:lock" .. msg.chat_id_) then
...
if UNTAGEER[2] == "الروابط" then
if redis:get(TAGEER.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الروابط \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:links:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "المواقع" then
if redis:get(TAGEER.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح المواقع \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:location:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "التاك" then
if redis:get(TAGEER.."tags:lock" .. msg.chat_id_) then
...
if UNTAGEER[2] == "الهاشتاك" then
if redis:get(TAGEER.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الهاشتاك \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:hashtag:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الجهات" then
if redis:get(TAGEER.."bot:contact:mute" .. msg.chat_id_) then
...
if UNTAGEER[2] == "الويب" then
if redis:get(TAGEER.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الويب \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:webpage:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "العربيه" then
if redis:get(TAGEER.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح العربيه \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:arabic:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الانظمام" then
if redis:get(TAGEER.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانظمام \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مفتوح سابقا\n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:member:lock" .. msg.chat_id_)
end
if UNTAGEER[2] == "الانكليزيه" then
if redis:get(TAGEER.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانكليزي \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:english:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "الاشعارات" then
if redis:get(TAGEER.."bot:tgservice:mute" .. msg.chat_id_) then
...
if UNTAGEER[2] == "الملصقات" then
if redis:get(TAGEER.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الملصقات \n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مفتوحه سابقا\n🎟┊ الأمر بواسطه » "..tmTAGEER(msg).."\n ‏ ", 1, "md")
end
redis:del(TAGEER.."bot:sticker:mute" .. msg.chat_id_)
end
if UNTAGEER[2] == "التوجيه" then
if redis:get(TAGEER.."bot:forward:mute" .. msg.chat_id_) then
...
if UNTAGEER[2] == "الفشار" then
if redis:get(TAGEER.."ffosh"..msg.chat_id_) then
...
redis:del(TAGEER.."keed_text" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_photo" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_video" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_Document" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_markdon" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_gif" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_audio" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_voice" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_link" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_user" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_hashtag" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_contect" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_arbic" .. msg.chat_id_)
end
...
redis:del(TAGEER.."keed_fosh" .. msg.chat_id_)
end
...
redis:set(TAGEER.."bot:sens:spam" .. msg.chat_id_, sensspam[2])
...
local kpbanned = redis:scard(TAGEER.."bot:banned:" .. msg.chat_id_) or "0"
redis:del(TAGEER.."bot:banned:" .. msg.chat_id_)
...
redis:setex(TAGEER.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 35, true)
end end
----------------------------------------------
...
local kpmomod = redis:scard(TAGEER.."bot:momod:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد الأدمنيه ≈ *"..kpmomod.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(TAGEER.."bot:momod:" .. msg.chat_id_)
end end
-----------------------------------------------
if txts[2] == "المدراء" and idf:match("-100(%d+)") then
...
local kpowners = redis:scard(TAGEER.."bot:owners:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المدراء ≈ *"..kpowners.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(TAGEER.."bot:owners:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local kpmonshis = redis:scard(TAGEER.."bot:monshis:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المنشئين ≈ *"..kpmonshis.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(TAGEER.."bot:monshis:" .. msg.chat_id_)
end end
--------DEL TAGEER_SUDO-----------------------------------------
...
local kpvipmem = redis:scard(TAGEER.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المميزين ≈ *"..kpvipmem.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
redis:del(TAGEER.."bot:vipmem:" .. msg.chat_id_)
end  end
---------------keeed del --------------------------------------------
...
local kpkeed = redis:scard(TAGEER.."bot:keed:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المقيدين ≈ *"..kpkeed.."*\n🌀┊ تم مسحهم بنجاح ✓\n‏", 1, "md")
local hash = "bot:keed:" .. msg.chat_id_
local list = redis:smembers(TAGEER..hash)
for k, v in pairs(list) do
redis:del(TAGEER.."bot:keed:" .. msg.chat_id_)
HTTPS.request("https://api.telegram.org/bot"..TAGEER_TOKEN.."/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..v.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
end end end
------------DEL vipmems-------------------------------------------
...
local kpmuted = redis:scard(TAGEER.."bot:muted:" .. msg.chat_id_) or "0"
redis:del(TAGEER.."bot:muted:" .. msg.chat_id_)
...
if redis:get(TAGEER.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local d = 0
if text:match("^1$") then
redis:del(TAGEER.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
for i = 1, NumberReturn do
getChannelMembers(msg.chat_id_, d, "Kicked", 200, kickedlist)
d = d + 200
end
if redis:get(TAGEER.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(TAGEER.."flood:max:" .. msg.chat_id_) then
flood_m = 5
else
flood_m = redis:get(TAGEER.."flood:max:" .. msg.chat_id_)
end
if not redis:get(TAGEER.."bot:sens:spam" .. msg.chat_id_) then
spam_c = 400
else
spam_c = redis:get(TAGEER.."bot:sens:spam" .. msg.chat_id_)
end
if redis:get(TAGEER.."warn:max:" .. msg.chat_id_) then
sencwarn = tonumber(redis:get(TAGEER.."warn:max:" .. msg.chat_id_))
else
sencwarn = 4
end
if redis:get(TAGEER.."floodstatus" .. msg.chat_id_) == "DelMsg" then
floodstatus = "المسح"
elseif redis:get(TAGEER.."floodstatus" .. msg.chat_id_) == "Kicked" then
floodstatus = "الطرد"
elseif not redis:get(TAGEER.."floodstatus" .. msg.chat_id_) then
floodstatus = "الطرد"
end
if redis:get(TAGEER.."warnstatus" .. msg.chat_id_) == "Muteuser" then
warnstatus = "الكتم"
elseif redis:get(TAGEER.."warnstatus" .. msg.chat_id_) == "Remove" then
warnstatus = "الطرد"
elseif not redis:get(TAGEER.."warnstatus" .. msg.chat_id_) then
warnstatus = "الكتم"
end
if redis:get(TAGEER.."bot:music:mute" .. msg.chat_id_) then
mute_music = "✅"
...
if redis:get(TAGEER.."bot:welcome" .. msg.chat_id_) then
send_welcome = "✅"
else
send_welcome = "✖️"
end
...
os.execute('wget https://raw.githubusercontent.com/alqaser/TAGEER/master/TAGEER.lua') 
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('TAGEER.lua')  
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
redis:setex(TAGEER.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
redis:set(TAGEER..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if redis:get(TAGEER.."lang:gp:" .. result.id_) then
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserTAGEER .. "]\n‏", 1, "md")
else
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserTAGEER .. "]\n‏", 1, "md")
end
chat_leave(result.id_, bot_id)
redis:srem(TAGEER.."bot:groups", result.id_)
else
...
local gp = tonumber(redis:scard(TAGEER.."bot:userss"))
gps = redis:smembers(TAGEER.."bot:userss")
...
if not redis:get(TAGEER.."dellmssg"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
if not redis:get(TAGEER.."lock_pinn"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم مسح الرد بنجاح\n ✓ ", 1, 'md')
redis:del(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(TAGEER..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(TAGEER..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(TAGEER..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(TAGEER..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(TAGEER..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(TAGEER..'rep_owner'..msg.content_.text_..''..msg.chat_id_..'')
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد' and is_owner(msg.sender_user_id_, msg.chat_id_)  then
redis:set(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','set_repgp')
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل لي كلمه الرد الان ❗️\n',1, 'md')
return false    end
if msg.content_.text_ then
local content_keep = redis:get(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_keep == 'set_repgp' then
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n' ,  1, 'md')
redis:set(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','save_repgp')
redis:set(TAGEER..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'',msg.content_.text_)
redis:sadd(TAGEER..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
-------------------------------------------------------------------------
if  msg.content_.text_ == 'الردود' and is_owner(msg.sender_user_id_, msg.chat_id_) then
local redod = redis:smembers(TAGEER..'rep_owner'..msg.chat_id_..'')
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(TAGEER..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(TAGEER..'gif_repgp'..v..msg.chat_id_)
redis:del(TAGEER..'voice_repgp'..v..msg.chat_id_)
redis:del(TAGEER..'stecker_repgp'..v..msg.chat_id_)
redis:del(TAGEER..'video_repgp'..v..msg.chat_id_)
redis:del(TAGEER..'text_repgp'..v..msg.chat_id_)
redis:del(TAGEER..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم مسح جميع الردود\n ✓ ", 1, 'md')
return false
end
...
redis:set(TAGEER.."add:repallt"..msg.sender_user_id_,'del_rep1')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد لمسحها ❗️\n",1, "md")
return false
end
if msg.content_.text_ then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم مسح الرد للكل\n ✓ ", 1, 'md')
redis:del(TAGEER.."add:repallt"..msg.sender_user_id_)
redis:del(TAGEER.."gif_repall"..msg.content_.text_)
redis:del(TAGEER.."voice_repall"..msg.content_.text_)
redis:del(TAGEER.."stecker_repall"..msg.content_.text_)
redis:del(TAGEER.."video_repall"..msg.content_.text_)
redis:del(TAGEER.."text_repall"..msg.content_.text_)
redis:del(TAGEER.."rep_sudo",msg.content_.text_)
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد للكل' and is_sudo(msg)  then
redis:set(TAGEER.."add:repallt"..msg.sender_user_id_,'set_rep')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد ❗️\n",1, "md")
return false    end
if msg.content_.text_ then
local content_keep = redis:get(TAGEER.."add:repallt"..msg.sender_user_id_)
if content_keep == 'set_rep' then
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n" ,  1, "md")
redis:set(TAGEER.."add:repallt"..msg.sender_user_id_,'save_rep')
redis:set(TAGEER.."addreply2:"..msg.sender_user_id_, msg.content_.text_)
redis:sadd(TAGEER.."rep_sudo",msg.content_.text_)
return false
end    end
------------------------------------------------------------------------------------
if  msg.content_.text_ == "ردود المطور" and is_sudo(msg) then
local redod = redis:smembers(TAGEER.."rep_sudo")
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(TAGEER.."add:repallt"..v)
redis:del(TAGEER.."gif_repall"..v)
redis:del(TAGEER.."voice_repall"..v)
redis:del(TAGEER.."stecker_repall"..v)
redis:del(TAGEER.."video_repall"..v)
redis:del(TAGEER.."text_repall"..v)
redis:del(TAGEER.."rep_sudo",msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmTAGEER(msg).."\n🎟┊ تم مسح ردود المطور\n ✓ ", 1, 'md')
return false
...
local list = redis:smembers(TAGEER.."bot:owners:" .. chattid)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(TAGEER.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
...
local kpmonshis = redis:scard(TAGEER.."bot:monshis:" .. chattid) or "0"
local kpbanned = redis:scard(TAGEER.."bot:banned:" .. chattid) or "0"
local kpowners = redis:scard(TAGEER.."bot:owners:" .. chattid) or "0"
local kpmuted = redis:scard(TAGEER.."bot:muted:" .. chattid) or "0"
local kpkeed = redis:scard(TAGEER.."bot:keed:" .. chattid) or "0"
local kpmomod = redis:scard(TAGEER.."bot:momod:" .. chattid) or "0"
local kpvipmem = redis:scard(TAGEER.."bot:vipmem:" .. chattid) or "0"
if not redis:get(TAGEER.."bot:group:link"..chattid) then
local getlink = 'https://api.telegram.org/bot'..TAGEER_TOKEN..'/exportChatInviteLink?chat_id='..chattid
local req = https.request(getlink)
local link = KPJS:decode(req)
if link.ok == true then 
redis:set(TAGEER.."bot:group:link"..chattid,link.result)
end
end
local lik_1 = redis:get(TAGEER.."bot:group:link"..chattid)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(msg.chat_id_, msg.id_, 1, "🕴┊ مدير عام عام » ["..owner.."]\n🥈┊ﮧ ["..title_name(chattid).."]("..(link or "t.me/TAGEER_ch")..")\n©️┊ عدد المدراء   » *"..kpowners.."*\n©️┊ عدد المنشئين   » *"..kpmonshis.."*\n🔆┊ عدد المدراء   » *"..kpmomod.."*\n🔰┊ عدد المكتومين  » *"..kpmuted.."*\n®️┊ عدد المحظورين » *"..kpbanned.."*\n®️┊ عدد المقيدين   » *"..kpkeed.."*\n🗯┊ عدد المميزين  » *"..kpvipmem.."*\n💠┊ﮧ `"..chattid.."`\n🏮┊ المطور » ["..sudo.."]\n✓", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
getChannelFull(chattid, gp_TAGEER_info, nil)
end 
end
------------UNPIN-----------------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
tdcli.sendDocument(TAGEER_SUDO, 0, 0, 1, nil, './TAGEER.lua', dl_cb, nil)
...
pinmsg(msg.chat_id_, pin_id, 0)
elseif redis:get(TAGEER.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(TAGEER.."lock_kickme"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🌀┊ امر اطردني معطل 🍃", 1, "md")
return false
end
redis:set(TAGEER.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:set(TAGEER.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ ارسل *{ نعم }* لطردك\n🚫┊ ارسل *{ لا } *لالغاء طردك", 1, "md")
end
local Kpkick = redis:get(TAGEER.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if Kpkick == "kick_mee" then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del(TAGEER.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(TAGEER.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ تم طردتك حياتي", 1, "md")
end 
end
end
if text:match("^لا$") then
local Kpunkick = redis:get(TAGEER.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
redis:set(TAGEER.."bot:viewget" .. msg.sender_user_id_, true)
...
redis:set(TAGEER.."addstart1"..msg.sender_user_id_, "theaddstarts")
return false
end end
if text then
local theaddstart = redis:get(TAGEER.."addstart1"..msg.sender_user_id_)
if theaddstart == 'theaddstarts' then
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم حفظ الكليشه 🍃", 1, 'md')
redis:del(TAGEER.."addstart1"..msg.sender_user_id_)
redis:set(TAGEER.."startbot", text)
return false
end end
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local get_me = function(extra, result)
local msgs = (tonumber(redis:get(TAGEER.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_)) or "0" )
local kptext = (tonumber(redis:get(TAGEER.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpsticker = (tonumber(redis:get(TAGEER.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local pkPhoto = (tonumber(redis:get(TAGEER.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVoice = (tonumber(redis:get(TAGEER.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpGif = (tonumber(redis:get(TAGEER.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpVideo = (tonumber(redis:get(TAGEER.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpSelfVideo = (tonumber(redis:get(TAGEER.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local kpcon = (tonumber(redis:get(TAGEER.."kpaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
--------------------------------------------------------
if text == ''..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' هينه' or text == ''..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' رزله' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(TAGEER_SUDO) then
send(msg.chat_id_, msg.id_, 1, 'انجب لك هذا مطوري العشق 😌💋', 1, 'md')
return false  end
if tonumber(result.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, 'لك مگدر اهين نفسي 😞😂', 1, 'md')
return false  end
local TAGEER = { "لك دايح ، احترم نفسك لا بال 👠😠","ها مصراع تراچي ، اگعد راحه تره روحي طالعه 😐🍃","ها ابن الحنينه، ليش متسكت وتنجب 🌚"}
send(msg.chat_id_, result.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
...
--------------------------------------------------------------------------------------------
if text == ''..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' بوسه' or text == ''..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(TAGEER_SUDO) then
...
return false  end
local TAGEER = { "اععع 🤢خده بي حب شباب الوصخ😹😹","موااح 💋 مواااح  حياتي💋😌🍃","💋😞نسخ لصق ع الشفه 👄"}
send(msg.chat_id_, result.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
--------------------------------- rdood bot --------------------------------------------------
if not redis:get(TAGEER.."lock_reeeep"..msg.chat_id_) then
if text == "السلام عليكم" or text == "سلام عليكم" or text == "سلام" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"وعليكم السلام والرحمه⇣😻","يمه هلا بالغالي 😻🍃","وعليكم السلام حبيبي ☺️🍃","كافي بس تسلمون 🌝💔" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {'اسمي '..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' 😌👌','تره اسمي '..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' يالجريذي ☹️😹'  }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == ''..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..'' then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {'اي نعم اني '..(redis:get(TAGEER..'TAGEERnams') or 'كيبر')..' 🍃😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-------------------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"هلووووات  ⁽🙆♂✨₎ֆ","يمه هلا بالعافيه 😻🍃","لا هلا ولا مرحبه شلونك مشتاقين 😻😂","اخلاً وصخلا 😌😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"تمام وانت/ي 😘🍃","شعليك انت 🧐😂","بخير انت/ي شلونك/ج ☺️","تمام وانت/ي ‏ ᵛ͢ᵎᵖ💛﴾" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"عٰٰٰود لوتُٰي 🙀 يٰگول بُِوت عبالٰه طافُٰٰي💔 ويضِٰل يمٰٓسلت وينشٰٰر روابٰٓط 😳🍃","اي بوت شتريد 😤" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"شكلولك عليه كاولي 😶😂","صوتي محلو للاسف 😌💔","اشعجب كاطع بيه ياراحتي النفسيه 😂💔","حرام  الغنا 😐🍃" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"بايات 💛🍃","گلعه 😶💔","الله الله الله وياك 😻😂","ثيمالا 🌝✋🏾" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"منو ال أجوو👀😹","جااااوات  ₎✿💥😈 ❥" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"نورك/ج هذا ورده 🌝🍃","بوجودك/غلا تسلم 😻✨","انت/ي اصل النور 😋🍃","عماني نورك 😣😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "بوسني" or text == "بوسه" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then local TAGEER = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"عيب 🙀😹","شبيه امك حمبي😋🍃" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"شبيه الشكره ام الوصخ 🤭😹","حبيته فدوووه😻","شرايد من خالته 🤭😂","خالته تفلش 😶😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"تف عليك ادبسزز 😒😹","لا تتفل على وجهك 😻😹","ما اسمحلك هيلگ 😡😹","بدون تفال رجائاً 😹😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"كافي ضحك 😐","لتضحك هواي وتصير فاول 🌝😹","هذ شبي يضحك 🙀😳","اضحك هيه الدنيا خربانه 😂😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"لصير عصبي يرتفع ضغطك 😌😂","صار وجه احمر مثل الطماطه 🙊😹","اوف شحلاتك وانت ضايج 😻","شبي هذا الله يستر 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😒" or text == "😒😒" or text == "😒😒😒" or text == "😒😒😒😒" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"لضوج متسوووه 🤭😹","يمه زعلان الحلو ما يكلي مرحبا 😻😹","اعدل وجهك لا اعدله الك/ج 😼👊🏼" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"يمه الگمر عذبني حبه 🙊😻","عو نضيف الوصخ 😹😹","طفي ضواك عميتني 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💋" or text == "💋💋" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"كبر 🙀 جان استحيتو 😹😹","عسل 😋🙊" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💔" or text == "💔💔" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"شبي مكسور 😔💔","موجوع كلبي والتعب بيه 😔😹","اكل بصل وانسه الحصل 😻😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😻" or text == "😻😻" or text == "😍😍" or text == "😍" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"شوفو الحب صاعد فول 😻😹","ها ناوي تزحف 😹😹","فدوووه لهاي العيون 🙊😻" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😐" or text == "🙂" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"شبيك حبيبي 😂💔","منور محمد الاعمى 😐😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"دووم الضحكه 🙊🍃","دوم الضحكه ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"اي غرد/ي دا اسمع 👂🏽😹","كول😹 (كول لو هدف)😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"انت/ي شبيك/ج😣","مابيه شي تسلم 💋😹" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
----------------------------------------
if text == "🌚💔" or text == "💔" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"هم اجانه محترك وجه😂♥️","هاي منو كاسر كلبك😡","اهو هم اجانه صخام🐸👌" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
---------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"ها بدت حبجيه ✨😂","لتلح عود يعني احبج🙈😹","كافي درينه مشتاقله 😒" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
---------------------------------------
if text == "😢" or text == "😢😢" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"يبجي دلوع😜😹","هاي عود انت جبير كاعد تبجي😑💔","لتلح درينه تبجي😒"}
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"علساس انت متفاعل😒😒","اي عندك اعتراض🤔","اني معليه احرسكم😎" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"عود اني حباب ونت شيطان يتعلم منك🙈😂😂","وجهك ميساعد🤢😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"اهو لا هلا بيه ✨😂","خي ولي مزاعله ✨😂","اهلا بيه بس اطرده اذا اجه😒😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
------------------------------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"وخرو وخرو🤓 هلا بلخميس تيرارا وياي يلا😍😹","هلا بلخميس عطله وكذا ركصو يلا😍😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "🙊" or text == "🙈" then
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"اول مره اشوف قرد يستحي🤔😂","ما مرجيه منك هايه صاير تستحي انته هوايه 😍😂😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(TAGEER..'lock:add'..msg.chat_id_) then
local TAGEER = {"ها ها يمعودين احنه هنا😒😹","اعتقد اكو خاص وخطبو وهنا زفه بسيارتي🚗😂" }
send(msg.chat_id_, msg.id_, 1,''..TAGEER[math.random(#TAGEER)]..'', 1, 'md')
end
end
end
...
if idf:match("-100(%d+)") and not redis:get(TAGEER.."bot:muted:Time" .. chat .. ":" .. user) and redis:sismember(TAGEER.."bot:muted:" .. chat, user) then
redis:srem(TAGEER.."bot:muted:" .. chat, user)
end
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(TAGEER.."bot:links:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.entities_ and result.content_.entities_[0] and (result.content_.entities_[0].ID == "MessageEntityTextUrl" or result.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(TAGEER.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.web_page_ and redis:get(TAGEER.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if (text:match("[Hh][Tt][Tt][Pp]") or text:match("[Ww][Ww][Ww]") or text:match(".[Cc][Oo]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Rr]")) and redis:get(TAGEER.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("@") and redis:get(TAGEER.."tags:lock" .. msg.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("#") and redis:get(TAGEER.."bot:hashtag:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("[\216-\219][\128-\191]") and redis:get(TAGEER.."bot:arabic:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
...
if redis:get(TAGEER..'bot:english:mute'..result.chat_id_) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end
end
if redis:get(TAGEER..'editmsg'..msg.chat_id_) == 'delmsg' then
local id = msg.message_id_
local msgs = {[0] = id}
local chat = msg.chat_id_
delete_msg(chat,msgs)
elseif redis:get(TAGEER..'editmsg'..msg.chat_id_) == 'didam' then
if redis:get(TAGEER..'bot:editid'..msg.message_id_) then
local old_text = redis:get(TAGEER..'bot:editid'..msg.message_id_)
...
redis:sadd(TAGEER.."groups:users" .. msg.chat_id_, msg.sender_user_id_)
if text then
...
redis:set(TAGEER.."Bot:KpBotAccount", data.value_.value_)
end end end end  end
----- End Source By TAGEER »»»»»»»»»»»»»»»»»»»
