...
tageer = Token:match("(%d+)")
redis:set(tageer..":VERSION",GetUser.information.Source_version)
redis:set(tageer..":SUDO_ID:",GetUser.information.id)
redis:set(tageer..":DataCenter:",GetUser.information.DataCenter)
redis:set(tageer..":UserNameBot:",BOT_User)
redis:set(tageer..":NameBot:",BOT_NAME)
redis:hset(tageer..'username:'..GetUser.information.id,'username','@'..GetUser.information.username:gsub('_',[[\_]]))
redis:set("tageer_INSTALL","Yes")
info = {}
...
tageer = Token:match("(%d+)")
our_id = tonumber(tageer)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(tageer..":UserNameBot:")
SUDO_ID = tonumber(redis:get(tageer..":SUDO_ID:"))
SUDO_USER = redis:hgetall(tageer..'username:'..SUDO_ID).username
version = redis:get(tageer..":VERSION")
DataCenter = redis:get(tageer..":DataCenter:")
...
if msg.type == "pv" and redis:get(tageer..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(tageer..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(tageer..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(tageer..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(tageer..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(tageer..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(tageer..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(tageer..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
if msg.Admin then redis:srem(tageer..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(tageer..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(tageer..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.tageer then 
if msg.text and ScriptFile.itageer then
for k, tageer in pairs(ScriptFile.tageer) do
local SearchText = msg.text:match(tageer)
if SearchText then
if not CheckFlood(msg) then
return false 
end
local GetMsg = ScriptFile.itageer(msg,{msg.text:match(tageer)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",tageer.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dtageer then
if ScriptFile.dtageer(msg) == false then
return false
end
...
if Plug.tageer then 
if msg.text and Plug.itageer then
for k, tageer in pairs(Plug.tageer) do
local SearchText = msg.text:match(tageer)
if SearchText then
if not CheckFlood(msg) then
return false
end
local GetMsg = Plug.itageer(msg,{msg.text:match(tageer)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",tageer.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dtageer then
Plug.dtageer(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source tageer")
end 

end
else
print("The File Script.lua Not Runing in The Source tageer")
...
function tdcli_update_callback(data)
	...
	redis:srem(tageer..'users',msg.sender_user_id_)
	end
	...
	download_file('https://github.com/TAGEER/TAGEER/blob/master/%E2%80%8Finc/run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/%E2%80%8Finc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/%E2%80%8Finc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/%E2%80%8Finc/functions.lua','./inc/functions.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |█████████████|\n\n🔝*¦* تم تحديث السورس الى اصدار *v'..redis:get(tageer..":VERSION")..'*\n📟*¦* تم اعاده تشغيل السورس بنجاح')
	dofile("./inc/Run.lua")
	...
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/inc/Run.lua','./inc/Run.lua')
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/inc/Script.lua','./inc/Script.lua')
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/TAGEER/TAGEER/master/inc/locks.lua','./inc/locks.lua')
	...
	if redis:get(tageer..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(tageer..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(tageer..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(tageer..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(tageer..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(tageer..'lock_bots'..msg.chat_id_)
	ISBOT = false
	ZISBOT = false
	...
	if ZISBOT and redis:get(tageer..'lock_bots_by_kick'..msg.chat_id_) then
	...
	if redis:get(tageer..'group:add'..data.chat_.id_) then
	redis:set(tageer..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(tageer..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(tageer..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(tageer..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	tageer..'group:name'..k,IDS.Title,
	tageer..'num_msg_max'..k,5,
	tageer..'group:add'..k,true,
	tageer..'lock_link'..k,true,
	tageer..'lock_spam'..k,true,
	tageer..'lock_webpage'..k,true,
	tageer..'lock_markdown'..k,true,
	tageer..'lock_flood'..k,true,
	tageer..'lock_bots'..k,true,
	tageer..'mute_forward'..k,true,
	tageer..'mute_contact'..k,true,
	tageer..'mute_document'..k,true,
	tageer..'mute_inline'..k,true,
	tageer..'lock_username'..k,true,
	tageer..'replay'..k,true
	)
	redis:sadd(tageer..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(tageer..'username:'..ID,'username',user)
	redis:sadd(tageer..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(tageer..'username:'..ID,'username',user)
	redis:sadd(tageer..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(tageer..'username:'..ID,'username',user)
	redis:sadd(tageer..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(tageer..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(tageer..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(tageer..'username:'..data.user_.id_).username
	...
	redis:hset(tageer..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
